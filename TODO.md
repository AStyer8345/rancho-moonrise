...
Last updated: 2026-05-29 (rancho-site-daily — quiet maintenance run; no shipped edits. **Third consecutive day** scheduled task fires against paused workstream (5/27 + 5/28 + 5/29). NEEDS ADAM #0 strengthened from 2-day to 3-day pattern. Re-Verify Gate 10/10 still_true (apex + www + sitemap + 3 landing pages + 3-page BlogPosting schema spot-check, 5/26 enrichment intact 3 days post-deploy). No new schema edits, no new NEEDS ADAM items.)

## 🔥 NEEDS ADAM (highest leverage this week)

0. **🆕 NEEDS ADAM — Rancho scheduled tasks have now fired against paused workstream THREE consecutive days (`rancho-content-weekly` 2026-05-27 + `rancho-site-daily` 2026-05-28 + `rancho-site-daily` 2026-05-29).** GOALS.md week of 2026-05-18 says "No Rancho Moonrise active work — paused (cruise control only if Ashley moves)" and none of these tasks are on the Keep-running list. Per CLAUDE.md rule ("Automated/scheduled tasks continue unless GOALS.md explicitly pauses them"), the spirit-of-the-pause says stop; the letter-of-the-pause does not explicitly name these tasks. **Pattern strengthens with 3 consecutive days across 2 distinct task IDs**, including `rancho-site-daily` firing 2 straight days against the cruise-control gate. Decision needed: (a) add `rancho-content-weekly` + `rancho-site-daily` (plus any other rancho-* tasks Adam wants explicitly paused) to the GOALS.md Pause List so future fires exit cleanly, (b) remove them from the scheduled-task registry entirely, or (c) flip Rancho off "paused" if cruise-control work like page strengthening + schema enrichment should keep running. All three runs (5/27, 5/28, 5/29) exited at the GOALS.md gate with no content/schema changes shipped — re-verify gates still_true on all three. The 30-second fix is option (a): one explicit pause-list bullet covers it. **Day-count drift will continue daily until Adam picks an option.**
...
2. **🆕 ResortPass listing exists — activate or formally kill (~30s with Ashley).** [resortpass.com/hotels/rancho-moonrise](https://www.resortpass.com/hotels/rancho-moonrise) is LIVE but dormant. Correct address (20117 Lockwood Rd), 3-star, pool hours 10am–6pm daily, listing copy mentions ranch views. Products section empty — "no active products at the moment." Either Adam/Ashley submitted previously and the listing went idle, or ResortPass created it speculatively. **Five weeks of "should we submit?" is the wrong question.** The actual decision is "add day-pass products and activate" OR "leave dormant and remove from TODO." A 30-second confirmation with Ashley settles this.
2. **GSC — request indexing on specific URLs (Priority 1 — refined target, ~5 min).** Apex is now indexing (11+ pages live in `site:ranchomoonrise.com`). The specific URLs that still need a manual indexing request are `/corporate-retreats/` (26 days uncrawled) and `/safari-tents-near-austin/` (31 days uncrawled). In Search Console: URL Inspection → Request Indexing for each. Don't re-submit the whole sitemap; target the specific URLs.
3. **Hipcamp strategy question for Ashley (carry-over from 2026-05-16 audit).** Read `brand/2026-05-16-hipcamp-curation-gap-audit.md`. TL;DR: Rancho is excluded from Hipcamp's Austin glamping editorial landing by 3 structural listing flags (`isPrivate: true`, `isReviewable: false`, state-level `discoverUrl`) + 8 content gaps. The audit refuses to prescribe "fix it" because `isPrivate: true` may be a deliberate Ashley choice. One question for Ashley: **"Is the Hipcamp listing intentionally private — SEO presence only — or do we want bookings from it?"** If intentional → close the Hipcamp "Gap" on improvement-plan. If not → execute Section 4B unlock sequence. Two voice/data fixes worth doing regardless: acreage drift ("34-acre" vs VOICE-GUIDE's 36) + voice violation ("unwind with a drink at our bar" — bar is event-only).
4. **Hotels.com listing copy fix (~15 min, 3rd week running).** Listing ho2867109568 surfaces "20 luxury cabins and safari tents that can accommodate up to 50 guests" in the brand SERP description. Per VOICE-GUIDE.md, no specific unit counts. Vercel site itself does NOT contain this copy — third-party listing problem.
...
## 📌 Competitive intel quick wins (refreshed 2026-05-25, day counts bumped 2026-05-26)

- **Rancho Moonrise ResortPass listing EXISTS, dormant — decision drift 8 days past 5/18 surface** ([resortpass.com/hotels/rancho-moonrise](https://www.resortpass.com/hotels/rancho-moonrise)). See NEEDS ADAM #2.
- **Google apex indexing continues to expand** — filtered `site:` queries now surface `/faqs/`, `/policies/`, `/weddings/`, `/contact/`, `/accessibility/`, `/event/july-4th/` in addition to 5/18 set. Landing pages still uncrawled (26d + 31d, +8d drift); see NEEDS ADAM #3.
