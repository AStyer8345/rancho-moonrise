...
Last updated: 2026-06-03 (rancho-content-weekly — same-day 2nd firing on top of this morning's rancho-site-daily. No shipped edits. Two task-spec hard stops re-verified still true (`brand/*testimonial*` missing; GOALS.md byline decision missing) — 5th structurally-blocked content-weekly run (4/22, 4/29, 5/06, 5/13, 6/03) on a **42-day pattern**. Per 5/13 commitment, today's strengthener-queue entry is a one-liner pointer, not a 5th re-litigation. **10th overall Rancho scheduled-task firing against the cruise-control gate across 4 distinct task IDs** since the GOALS.md week-of-5/18 pause; `rancho-content-weekly` itself is now at its 2nd cruise-control firing. Re-Verify Gate clean (apex 200 + server: Vercel + x-vercel-cache: HIT; www 308 → apex; sitemap 200). Queue table: `accommodations.html` Last Touch 5/13 → 6/03 (still BLOCKED). NEEDS ADAM #0 30-second pause-list fix is now **7 days overdue**. Run-log: `run-logs/strengthener-queue.md` + this morning's `run-logs/2026-06-03-seo.md`.)

## 🔥 NEEDS ADAM (highest leverage this week)

0. **NEEDS ADAM — Rancho scheduled tasks have now fired against paused workstream TEN consecutive firings across 4 distinct task IDs (`rancho-content-weekly` 5/27 + `rancho-site-daily` 5/28 + `rancho-site-daily` 5/29 + `rancho-competitive-weekly` 6/1 + `rancho-review-monitor` 6/1 RUN_041 + `rancho-site-daily` 6/1 + `rancho-site-daily` 6/2 + `rancho-site-daily` 6/3 + `rancho-content-weekly` 6/3 = 5th site-daily firing + 2nd content-weekly firing, same-day pair).** GOALS.md week of 2026-05-18 says "No Rancho Moonrise active work — paused (cruise control only if Ashley moves)"; none of these tasks are on the Keep-running list. Per CLAUDE.md rule ("Automated/scheduled tasks continue unless GOALS.md explicitly pauses them"), the spirit-of-the-pause says stop; the letter-of-the-pause does not explicitly name these tasks. **Pattern strengthens further with the same-day content-weekly firing (6/3 PM, this run) — `rancho-content-weekly` is now at its 2nd cruise-control firing alongside `rancho-site-daily`'s 5th, and today is the first calendar day with TWO Rancho cruise-control firings (site-daily AM + content-weekly PM). Also: content-weekly hits TWO LAYERS of pause signal — both the broad GOALS.md week-of-5/18 pause AND the task-spec's own hard stops (testimonials list + byline decision), unresolved for 42 days now.** Note on `rancho-review-monitor` specifically: it produces no Adam-facing artifacts beyond carry-forwards already raised (Cassie + Haylee drafts, both surfaced for 16 days now) and live state JSON; arguably the cleanest cruise-control candidate of the four. Decision needed: (a) add `rancho-content-weekly` + `rancho-site-daily` + `rancho-competitive-weekly` + `rancho-review-monitor` (plus any other rancho-* tasks Adam wants explicitly paused) to the GOALS.md Pause List so future fires exit cleanly, (b) remove them from the scheduled-task registry entirely, or (c) flip Rancho off "paused" if cruise-control work should keep running. The 30-second fix is option (a): one explicit pause-list bullet covers it. **The 30-second fix has now been on Adam's plate since 5/27 — 7 full days. Firing-count drift will continue daily until Adam picks an option** — next `rancho-site-daily` fires 6/4 morning unless paused; next `rancho-content-weekly` fires 6/10 unless paused.

1. **🆕 POST 2 REVIEW REPLIES — drafts ready in `brand/review-reports/2026-05-19-review-report.md`. DAY 14 UNPOSTED IN MONITOR.** (a) **Cassie Butterfield (Google 5★, ~16 days old, ~2026-05-16)** — corporate-retreat review, drafted reply is voice-clean and ready to paste as-is in GBP dashboard, 30 seconds. (b) **Haylee L. (The Knot 1★, 2026-02-26, ~95 days / ~13.5 weeks unreplied)** — quiet-property-risk complaint from a site visit, drafted reply has an edit-flag on the "we're flagging neighboring-property activity..." sentence — 2-minute decision on whether to publicly commit to that operational change before posting. **Visibility cost rising:** Haylee's reviewer name + verbatim body text now surfaces in direct Rancho-attributed Google snippets for the 3rd consecutive run; bad-review exposure is widening week-over-week, raising the cost of continued non-reply. **After posting, mark `rancho-review-replies-2026-05-20` done from the briefing page** so this monitor can stop carrying it forward.
2. **ResortPass listing exists — activate or formally kill (~30s with Ashley).** [resortpass.com/hotels/rancho-moonrise](https://www.resortpass.com/hotels/rancho-moonrise) is LIVE but dormant. **14-day decision drift since 5/18 surface.** Correct address (20117 Lockwood Rd), 3-star, pool hours 10am–6pm daily. Products section empty. Decision: "add day-pass products and activate" OR "leave dormant and remove from TODO." **6/1 new context:** Lucky Arrow's dormant ResortPass listing publishes operating hours (Fri–Sun 2–6pm) despite 0 products — Rancho could ship operating hours as a no-product middle ground if undecided.
2. **GSC — request indexing on specific URLs (Priority 1, ~5 min).** `/corporate-retreats/` (**31 days uncrawled**, +7d from 5/25) + `/safari-tents-near-austin/` (**36 days uncrawled**, +7d). URL Inspection → Request Indexing for each. **6/1 new competitive pressure:** Camp Lucy now in corporate-retreat top 10 at `camplucy.com/meetings-events/corporate-packages` — name-brand entrant in the cluster Rancho's `/corporate-retreats/` is built for.
...
4. **Hotels.com listing copy fix (~15 min, 4th week running).** Listing ho2867109568 surfaces "20 luxury cabins and safari tents that can accommodate up to 50 guests" in brand SERP description. Per VOICE-GUIDE.md, no specific unit counts. Vercel site itself does NOT contain this copy — third-party listing problem.
5. **Blog pipeline restart — framing further degraded.** Spoon Mountain's `/romantic-weekend-getaways-near-austin/` SERP pattern crystallized at 50/50 over 4 weeks (in 5/11, out 5/18, in 5/25, out 6/1). The 5/11 "competitors are now winning research-intent SERPs" framing is now confirmed weak — research-intent content can land but doesn't hold. Does NOT escalate restart.
6. **Glamping Hub submission.** 7 weeks running. Free, 15 min, glampinghub.com/list-your-property.

## 📌 Competitive intel quick wins (refreshed 2026-06-01)

- **Rancho Moonrise ResortPass listing EXISTS, dormant — decision drift 14 days past 5/18 surface** ([resortpass.com/hotels/rancho-moonrise](https://www.resortpass.com/hotels/rancho-moonrise)). See NEEDS ADAM #2. New 6/1 middle-ground option: publish operating hours like Lucky Arrow does, without committing to products.
- **🆕 Lucky Arrow Retreat now in Hipcamp's curated "20 Best Glamping Near Austin"** at position 8. First direct-competitor in Hipcamp editorial set across 3 reads (5/16 / 5/25 / 6/1). Rancho remains absent. Reinforces Hipcamp Ashley question (NEEDS ADAM #3).
- **🆕 Camp Lucy** (Driftwood winery + chapel + accommodations) new on corporate-retreat top 10 at `camplucy.com/meetings-events/corporate-packages`. Name-brand entrant in the cluster Rancho's `/corporate-retreats/` is built for.
- **🆕 Spoon Mountain weekend-getaways page OUT of top 10 again** — 4-week pattern crystallized as in/out/in/out. 50/50 hit rate. Page content unchanged; SERP slot genuinely unstable.
- **🆕 Safari for the Soul unit count is genuinely ambiguous** — site classifies Sabi Nights as "Safari Tent/Yurt"; honest read 1–2 safari tents + 3–4 yurts + 1 residential at `safariforthesoulglamping.com`. ±1 tolerance going forward.
- **🆕 Hipcamp carousel substantially rotated** — only 6 of 20 properties overlap with 5/25 set. Lucky Arrow's inclusion is the material change.
- **🆕 viprealestate.com** "Best Seven Glamping Getaways Near Austin, Texas" listicle new on weekend-getaway SERP.
- **Lucky Arrow ResortPass dormant — but now publishes Fri-Sun 2-6pm operating hours** despite 0 products. Middle-ground pattern.
- **Google apex indexing — continued expansion via filtered `site:` queries**; landing pages still uncrawled (31d + 36d, +7d drift each). See NEEDS ADAM #3.
- **Walden Retreats** — page describes "four accommodation types" with only three named — minor textual inconsistency, not a state change.
- **The Yurtopian** corp-retreats page still off SERP (3rd consecutive week).
- **Camposanto ATX** holds glamping-near-austin top-10 slot (2nd consecutive week).
- **Track Loving Heart Retreats** — domain still has TLS cert mismatch, not fetchable.
- **Track Green Acres ATX (Elgin) as closest geographic competitor.**
