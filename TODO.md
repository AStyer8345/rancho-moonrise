...
Prior: 2026-09-09 (rancho-site-daily — **Found and fixed a second SERP meta-description overflow the 9/7 pass missed: `safari-tents-near-austin.html` at 163 chars**, trimmed to 129, no voice violations, `validate:site` passes. Re-Verify Gate 2/2 still_true: `/safari-tents-near-austin/` still absent from the same 8-owned-URL `site:` result as 8/31/9/7 (on-site diagnosis stays closed), 5-file NEEDS OWNER set unchanged. **New, not this task's — flagged below:** `rancho-review-monitor`'s 2026-09-07 RUN_073 four-file update was found sitting uncommitted at run start (same "shipped, never committed" pattern hit 3+ times before, this time on review-monitor); its `CONTEXT.md` hunk was isolated and reapplied intact rather than swept into this commit, and its other 4 files remain untouched. A concurrent `rancho-content-weekly` session was also actively writing `TODO.md`/`site/pages/pool-day-pass-austin.html`/`site/sitemap.xml` during this run — worked around with explicit pathspecs, nothing of theirs touched. Run-log: `run-logs/2026-09-09-seo.md`. Prior header ↓.)

Prior: 2026-09-07 (rancho-competitive-weekly — **ResortPass access restored after two blocked reads; day-pass underpricing finding re-verified live, not held.** Rancho unchanged ($20/$15, 4.8★/53); Lucky Arrow's rating held (4.6★) but review count moved 201→209 (+8) during the blackout, plus a new $75/4-guest bundle. Logged `resortpass-access-blocked-2026-08-31` RESOLVED in `rancho-done-log.md`. **Broad-glamping regression (3→1 owned URLs, flagged 8/31 as one read) confirmed via a 2nd read of the identical query** — same reduced count, six days apart; treat as a settled floor, not worsening. **New, snippet-level only:** two independent search reads attribute the same banned Knot sentence ("20 luxury cabins...50 guests") to Rancho's WeddingWire listing — direct fetch still won't render the page, so this needs a human to actually confirm inside the WeddingWire dashboard, not another automated attempt. The Knot itself reproduced verbatim a 4th read; Hotels.com still hasn't in 5. Brand snippet clean for a 2nd consecutive read (watched, not claimed — The Knot's own listing is still dirty this same run). Hipcamp curated absence 11th read (Lucky Arrow #8, Ranch 3232 #16 unchanged); Glamping Hub ~21 weeks absent. Serana re-confirmed no published price. Intel card inserted byte-identically into both improvement-plan files, verified at `origin/main` on both repos and both live deployed URLs before calling it shipped — the exact check the 8/25 report skipped. Gate: 8 still_true, 2 blockers resolved, 1 confirmed-4th-read, 1 new snippet-level finding. Run-log: `run-logs/2026-09-07-competitive.md`. Prior header ↓.)

Prior: 2026-09-07 (rancho-site-daily — **Found and fixed a real on-page defect: 4 pages' SERP meta descriptions ran past Google's ~155-160 char truncation point** (blog.html, faqs.html, videos.html, wedding-venues-near-austin.html — 167-180 chars each). Trimmed to 121-132 chars, no voice violations, `validate:site` passes, committed `4f4534f`, pushed, confirmed live on all 4 URLs. This is a distinct lever from the internal-link/word-count/schema/title-match set already measured and disconfirmed as ranking factors — SERP truncation affects CTR, not ranking, and hadn't been checked before. Re-Verify Gate 2/2 still_true: `/safari-tents-near-austin/` still unindexed, 5-file NEEDS OWNER set unchanged. Nothing below changed as a result of this run. Run-log: `run-logs/2026-09-07-seo.md`. Prior header ↓.)

Prior: 2026-09-05 (rancho-site-daily — **Quiet cruise-control run.** Repo clean (HEAD == `origin/main`), site health clean (30/30 sitemap URLs 200), Re-Verify Gate 2/2 still_true — `/safari-tents-near-austin/` still unindexed (~131 days) and the 5-file NEEDS OWNER uncommitted set unchanged. No on-page defect found today and none manufactured to fill the slot; the standing "do not propose on-page passes as default work" instruction holds. Nothing below changed as a result of this run. Run-log: `run-logs/2026-09-05-seo.md`. Prior header ↓.)

Prior: 2026-09-04 (rancho-site-daily — **WeddingWire's `sameAs` contradiction, open since 2026-08-19, is resolved.** Excluded three times (8/19, 8/25, 8/31) for lack of independent confirmation — resolved by varying the query rather than retrying the known-403 direct fetch: `Rancho Moonrise weddingwire.com` returned `https://www.weddingwire.com/biz/rancho-moonrise/f38c5e35e5491216.html`, independently reconfirmed by a second query, cross-matched to the property's operational address. Added to `site/index.html`'s `sameAs` (14 → 15), `validate:site` passes. **This still leaves two open items from the "three never-published 8/25 quick wins" line further down: claim the WeddingWire vendor listing, and fix The Knot description — those are unchanged, not this run's to do.** **Today's assigned top-autonomous-slot item — "extend AEO measurement to a second answer engine" — was attempted and closed negative.** Tried `WebFetch` (a genuinely different mechanism than the bare-`curl` path that failed 8/17) against Bing and Google directly: both failed the mandatory brand-canary guard identically to the 8/17 bare-curl failure (Bing returned gorancho.com/Wikipedia/Merriam-Webster instead of the site; Google returned an error interstitial). Logged as **MEASUREMENT BLOCKED, not "0 citations"** per the shared runbook — this closes the "maybe a different fetch mechanism works" hypothesis for good. **Genuine second-engine AEO measurement needs a paid API key** (ChatGPT, Perplexity, or Bing Web Search API) — nothing short of that will unblock it. Extended the shared `client-ops/templates/re-verify-before-report.md` dead-paths table with this finding. **New finding, own remit: Apple Maps lists hours as "3:00 PM – 5:00 PM daily"** — re-read via `WebFetch`, cross-verifying review-monitor's 8/30 rating figures (80%, 5 ratings) exactly along the way. A 2-hour daily window is almost certainly wrong for an events venue and risks Siri/Spotlight telling people the property is closed. Flagged below as a new NEEDS ADAM item (Apple Business Connect fix, ~2 min) — not fixable from this repo. Everything else re-verified unchanged: site health, `/safari-tents-near-austin/` still uncrawled (still closed on the on-site side per the standing 8/31 instruction), and the 5-file NEEDS OWNER uncommitted set (untouched, still not this task's). Gate: 4 claims — 1 resolved, 3 still_true. Run-log: `run-logs/2026-09-04-seo.md`. Prior header ↓.)
...
## ~~NEEDS OWNER (not site-daily's) — review-monitor's 2026-09-07 RUN_073 update sat uncommitted~~ → ✅ **RESOLVED 2026-09-09 by `rancho-review-monitor` RUN_074**
...
- [ ] `/Users/adamstyer/Documents/rancho-moonrise` is a **detached HEAD in an interrupted `git rebase`** (last touched 2026-09-18 10:38; nothing running). `main` ref = `ab1b210`: **8 unpushed commits** (7 notebooklm-sync + `rancho-review-monitor` RUN_078 `ab1b210`) and 10+ behind `origin/main`. **Update 2026-09-21:** RUN_078's content already landed on `origin/main` as `1ffb25a`; only the 7 notebooklm-sync commits are truly stranded. notebooklm-sync flagged it 9/18 and correctly refused to touch it; site-daily did the same and worked from a worktree off `origin/main` instead. Fix = someone finishes (`git rebase --continue`) or aborts it and pushes; nothing is at risk of loss (`main` ref + reflog hold all 8). ~~Review-monitor's RUN_078 commit isn't on `origin/main`~~ — resolved 2026-09-21 (landed as `1ffb25a`).
- [ ] **The 5-file NEEDS OWNER set is now in `stash@{0}`** ("pre-existing NEEDS OWNER uncommitted set", 9/18 07:38): `api/inquiry.js`, `site/css/styles.css`, `site/js/main.js`, `site/pages/contact.html`, `site/pages/weddings.html` (−44, the wedding guest-accommodations deletion) — byte-identical to the 8/31 baseline. Still needs owner review before anything is applied or published. Stash stack is shared across worktrees: use `git stash show -p stash@{0}`, never bare `pop`.
- File-sync duplicates (`CHANGELOG 2.md`, `site/pages/weddings 2.html`, etc.) are still on disk in the main checkout; gitignored via `* [2-9].*` (not tracked, not crawlable). Not actioned.
...
- [x] Verify the production deployment and record live Lighthouse results in `docs/website-audit-2026-09-16.md`.
...
### NEEDS ADAM — From 2026-09-24 Bi-Weekly Audit

Source: `site/audits/2026-09-24-business-audit.html`. Ordered by impact. None of these need code. Together they're about a one-hour owner sprint, and doing them moves Reviews D → C+, GBP B- → B+, Operations B → B+ next audit.

1. **Post the 2 drafted replies (~5 min).** Cassie Butterfield Google 5★ (paste-ready) + Haylee L. The Knot 1★ (204+ days unreplied; needs the mitigation-wording call). Drafts: `brand/review-reports/2026-05-19-review-report.md`. Unposted 122+ days.
2. **Read + answer the unmanaged review surfaces (~15 min).** Airbnb safari-tent 3★ ×2 (unread), Facebook negative (86%, text unread), Expedia 6 (reply coverage never checked). Paste text into the repo so drafts can be written.
3. **Resolve the stuck main checkout (~10 min).** Detached HEAD mid-rebase since 9/18; the 9/20 metrics commit `157efd1` + 7 notebooklm-sync commits aren't on `origin/main`. Finish or abort the rebase, push, then decide the `stash@{0}` 5-file set (includes the −44-line wedding-page deletion).
4. **Supply the GA4 / GTM ID (~2 min).** `ga4MeasurementId: ''` still blank live 9/24. Conversions report nowhere.
5. **Claim TripAdvisor (`d33307272`) + fix third-party copy (~30 min).** TripAdvisor 0/unclaimed; Hipcamp "34-acre"/"bar"; The Knot unit-count sentence (upstream of Yodel + brand snippet). Confirm WeddingWire claim status.
6. **Apple Business Connect hours (~2 min).** Apple Maps says 3–5 PM daily → Siri/Spotlight say "closed."
7. **GSC URL Inspection on `/safari-tents-near-austin/` (~2 min).** ~150 days unindexed; on-site causes ruled out 8/31.
8. **Site copy decisions (one each):** dead "Escape to Rancho Moonrise" card on `/videos/` (thumbnail 404; delete or give a new video ID); homepage "125+ Google reviews" + schema `reviewCount: 125` vs last read 130 (export the current GBP count first); `/host-your-event/` capacity 200 vs "1,000+".
9. **Test a $25–30 ResortPass day pass (Ashley).** Rancho is $20 at 9.4/10 vs Lucky Arrow $35 at 9.2/10.
10. **Corporate-retreat HoneyBook flow.** Corporate is now the best-ranking, first-cited query; inquiries still go into the wedding template. Third audit with no HoneyBook movement.
...
3. **PARTIAL (2026-09-24 audit):** `/corporate-retreats/` indexed and ranking; `/safari-tents-near-austin/` still unindexed — carried in the 09-24 section. ~~**GSC Request Indexing on the 2 uncrawled landing pages (~5 min).**~~ `/corporate-retreats/` (~59d) + `/safari-tents-near-austin/` (~64d) are built and schema-complete but uncrawled — rest of site indexes fine (homepage #1 on brand), these two lag specifically.
...
5. ✅ **RESOLVED 2026-09-16 (confirmed by 2026-09-24 audit)** — expired April/May homepage event fallbacks removed; HTML now falls back to an evergreen calendar link. (Hero "125+" review copy still open — carried in the 09-24 section.) ~~**Fix the stale homepage "Upcoming Events" block.**~~ The live homepage shows past dates (April 26, May 2, May 29) as "upcoming" — a visitor skimming for "what's on this weekend" reads stale events. Refresh or auto-wire it to the live Supabase feed. (Also: hero still says "125+ Google reviews" vs ~130 live.)
...
## ✅ RESOLVED (re-verified 2026-09-21) — 🔧 Schema gaps found 2026-07-22 (per-page JSON-LD matrix)

> **Live matrix 2026-09-21, all 31 sitemap pages:** every page has BreadcrumbList + a WebPage-family type + speakable, 0 parse errors — except `/website-credits/` (new 9/19–20), which got BreadcrumbList today (speakable deliberately omitted). Items 1–3 below (videos, accessibility, policies) were closed by 8/16 (`rancho-seo-s4` RESOLVED then). **Do not re-surface.** Kept for history.
...
- **1️⃣ Claim + optimize the WeddingWire listing (Ashley or Adam, ~15 min).** `weddingwire.com/biz/rancho-moonrise/f38c5e35e5491216.html` — listing existence confirmed on a third independent read 2026-09-07. **New reason to prioritize this one:** two independent search reads 9/7 returned The Knot's exact banned sentence ("20 luxury cabins and safari tents for up to 50 guests") attributed to this WeddingWire listing — snippet-level evidence only, direct fetch still won't render the actual page. **Whoever logs in should check the description text directly and screenshot it** — this report cannot render the page to confirm or rule it out.
- **2️⃣ Fix the venue description on The Knot (Ashley or Adam, ~15 min).** **Confirmed a 4th time on 9/7**, verbatim: *"Rancho Moonrise contains 20 luxury cabins and safari tents for up to 50 guests"* — alongside the same listing's *"can host 300+ guests."* Four defects in one sentence: banned word "luxury", banned unit count, "50 guests" contradicting the site's own `maximumAttendeeCapacity: 200`, and a **6× self-contradiction**. Hotels.com again did **not** reproduce the phrase (5th clean read) — attribution stays The Knot confirmed, Hotels.com clear. **If WeddingWire turns out to carry the same text (see #1), the two may share one vendor-data source — fixing one may not fix the other; check both after editing either.**
- **3️⃣** ~~Add WeddingWire to the site's `sameAs` array~~ → ✅ **DONE 2026-09-04 by `rancho-site-daily`** (14 → 15).
- **4️⃣ `/safari-tents-near-austin/` — GSC request-indexing (Adam, ~2 min).** ~**131 days** per `rancho-site-daily`'s 2026-09-05 read (not independently re-measured by this task this week). Entire remaining GSC ask. (Owned by `rancho-site-daily` + Adam.)
- **5️⃣ Glamping Hub submission — ~21 weeks absent.** Free, ~15 min at `glampinghub.com/list-your-property`. Glamping Hub holds **#1 or #2 on three tracked head terms**; the only route onto an aggregator's SERP is to be in the aggregator.
- **6️⃣ Hipcamp curation question for Ashley — 11th consecutive absence.** *"Is the Hipcamp listing intentionally private — SEO presence only — or do we want bookings from it?"* Lucky Arrow holds curated **#8**; Ranch 3232 **#16** — both exactly unchanged from 8/31. Rancho has never appeared.
- **❌ RETIRED 2026-08-31, RE-CONFIRMED 2026-09-07 — "publish flat buyout pricing".** Serana still publishes no price on direct fetch. One caveat noted, not acted on: Serana's name resurfaced 9/7 in a synthesized answer for the corporate head term, described by amenities only, no price, no URL/rank placement. **Do not re-propose without a real published-price sighting.**
- **✅ UNFROZEN 2026-09-07 — day-pass underpricing, now live-verified.** Rancho **$20 / $15 half-day at 4.8★/53** (unchanged) vs Lucky Arrow **$35 at 4.6★/209** (review count +8 since 8/25) — still roughly 43% below market at a higher rating. **ResortPass access restored 9/7** after two blocked reads (review-monitor 8/30, this task 8/31) — both values re-verified by direct fetch, not held on faith. Usable in a pricing conversation with current data. → Ashley.
...
- [ ] **Paste the Facebook non-recommend review text into the repo** so a response draft can be written. Blocked on the NEEDS ADAM item above. **Updated 2026-09-18 by RUN_078:** the aggregate figure (6/86%) is readable again via WebSearch as of this run — `facebook-aggregate-verification` blocker resolved — but the review **text** itself has never been surfaced since RUN_068. Still the same 60-second Page fix.
- [ ] **Post the two drafts sitting unposted, now day 122** — Cassie Butterfield (Google 5★) and Haylee L. (The Knot 1★), both in `brand/review-reports/2026-05-19-review-report.md`. Haylee's 1★ is now **204 days / 29.1 weeks unreplied**. Re-verified against the root done-log 2026-09-18: last review-reply resolution is still 2026-04-15.
