...
- [ ] Extend AggregateRating to remaining 6 landing pages — `glamping-near-austin-texas`, `things-to-do-manor-tx`, `glamping-vs-camping`, `things-to-do-near-austin-with-kids` (BlogPostings — same publisher pattern), and `faqs` + `contact` (utility pages — needs schema decision). Next SEO run.
