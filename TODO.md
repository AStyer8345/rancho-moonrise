...
### NEEDS ADAM — one Google Business Profile URL (~30 seconds) — added 2026-08-19 by `rancho-site-daily`
...
### NEEDS ADAM — Six reviews exist on Expedia and nobody has ever checked whether they have replies (~2 min) — added 2026-08-19 by `rancho-review-monitor`
...
**Two caveats recorded rather than smoothed.** The count came from a search snippet, not a direct scrape (the direct fetch returned **HTTP 429 for the second consecutive run**), so it has **not** been promoted to the authoritative field. And Facebook also reads exactly 6 — neither query's result set contained `facebook.com` and both bound the 6 to OTA descriptors, but a third differently-phrased confirmation is wanted before this becomes a hard number.
...
### NEEDS ADAM — Google's review count is 92 days old, and the fallback is quoting our own website back to us (~30 seconds) — added 2026-08-19 by `rancho-review-monitor`

The last **authoritative** Google review count — 130 at 4.9★ — came from a Chrome MCP read of the GBP dashboard on **2026-05-19**. That is **92 days ago**, and it is the only real Google number this task holds.
...
### NEEDS ADAM — ResortPass has 53 reviews and nobody is watching it (~2 min) — added 2026-08-17 by `rancho-review-monitor`, **re-verified 2026-08-19 (2nd run carrying this)**

> **RUN_070 status:** still open, still out of scope. **The rating block did not render this run** — the page returned HTTP 200 with products (`Half-Day Pass from $15`, `Day Pass from $20` — 2, unchanged), star class and pool copy all identical, but no `4.8` and no `(53)` survived the fetch. That is an **extraction failure, not a rating drop**, and it was deliberately not recorded as one: reporting "no rating found" as a decline, on the one platform already showing this year's only rating decline, is exactly the false negative the Re-Verify Gate exists to stop. **53 @ 4.8★ is held `STALE:2026-08-18`**, backed by three prior reads (this task 8/17, `rancho-competitive-weekly` 8/17, this task 8/18). Re-read next run before anyone draws a conclusion from it.
...
## 📌 Competitive intel quick wins (refreshed 2026-08-17)

- **🎉 RESOLVED 2026-08-17 — Rancho is ranking on non-brand keywords for the first time.** Three owned URLs surface on commercial queries: `/blog/corporate-retreat-near-austin/` ~#4 (two separate corporate queries), `/corporate-retreats/` ~#6, `/blog/weekend-getaways-near-austin/` ~#4–5 (two weekend-getaway queries). Confirmed across 3 independent live queries. The 0/4 claim carried since April is dead. (done-log)
- **🎉 RESOLVED 2026-08-17 — `/corporate-retreats/` is INDEXED.** `site:` query returns it as top result; ranks ~#6. The #1 Adam-side GSC action across 8 consecutive reports is closed. **The GSC ask narrows to one URL.** (done-log)
- **🆕 THE BLOG DID IT, NOT THE LANDING PAGE.** On the head corporate term the blog post (~#4) outranks the purpose-built landing page (~#6); weekend-getaway is carried entirely by a blog URL. Both happened **while the blog pipeline has been paused since 2026-04-23**. The "publish own-domain guide content" thesis is now demonstrated on this property, not inferred from competitors. **Strongest argument yet for revisiting the pause with Ashley.**
- **🆕 PRICING HEADROOM QUANTIFIED — Rancho day pass is 43% below market.** Rancho $20 adult / $15 half-day at **4.8★ / 53 reviews**. Lucky Arrow **$35** (raised from $30 this cycle) at **4.6★ / 200 reviews**. Higher rating, 57% of the price, and the competitor just proved the market absorbs an increase. Revenue lever requiring a decision, not a build. → Ashley.
- **⚠️ RESORTPASS REGRESSION — Rancho's experience products disappeared.** 7/15 listing had 4 products; today it has **2** (Half-Day $15, Day Pass $20). **Pilates and Horses ($58) and Full Moon Floating Sound Bath ($30) are gone.** Both map to known recurring ranch programming → reads as a lapse, not a delisting. Nothing logged in-repo either way. Review surface 4.9★/45 → **4.8★/53**. → 30-second Ashley question. (done-log — prior claim corrected)
- **🆕 Three new corporate-retreat entrants, one sharing Rancho's geographic hook.** **Serana** (Paige TX, 8 units, **$1,000/night flat full-property weekday**, Starlink/sauna/cold plunge/pool) ~#3. **7744 Ranch** markets "20 minutes from downtown, just east of Austin" — **Rancho's exact positioning line** — at 100 guests / 10 overnight vs Rancho's 200 / full-ranch lodging. **Artemis Ranch** also entered. Cluster tightened in the same cycle Rancho entered it.
- **🆕 NEW GAP — published buyout pricing.** Serana publishes $1,000/night full property; Lucky Arrow publishes $545–650 pp/night corporate. Rancho custom-quotes everything. A "starting at" band removes a comparison-shopping friction point on the exact term Rancho now ranks for. Copy decision for Ashley, not an SEO task.
- **✅ RESOLVED — Spoon Mountain pricing inconsistency.** Page **rebuilt as a 10-property roundup** (new H1, "10 Seasonal Picks (2026 Guide)"); no `$245` figure anywhere. Still 3 tents, still top property slot. The competitor upgraded the exact page format we recommend Rancho adopt. (done-log)
- **✅ RESOLVED — Safari for the Soul unit count.** Settled at **5 glamping units** (Marble Falls 4: Cosmic Nights + Sabi Nights safari tents, Moonlight Magic + Starry Nights yurts; Spicewood 1: Tentation Yurt) **plus 1 separate residential** (KickBack Cove Home). The long-carried "6" was double-counting the house. (done-log)
- **`/safari-tents-near-austin/` is the LONE uncrawled holdout — ~113 days (+33d).** Targeted `site:` on that exact path never returns it. This is the entire remaining GSC ask. (Owned by `rancho-site-daily` + Adam.)
- **🔄 Hipcamp curated set RE-STABILIZED (8/17)** — top 8 byte-identical to 7/15, Lucky Arrow holds **#8**. Rancho absent **8th consecutive read**. Reinforces the Hipcamp Ashley question.
- **🔄 Glamping head term — aggregators retook the top 3** (Outdoorsy #1, Glamping Hub #2, Hipcamp #3); Talula Mesa slid #1 → ~#4 (confirmed 4 tents / 15 acres / Marble Falls, 55 min from downtown).
- **🔄 Lucky Arrow re-entered the corporate SERP ~#2** (was off the top 10 on 7/15), publishing $545–650 pp/night and claiming 40+ corporate retreats annually. Sage Hill holds #1.
- **🆕 New safari-tent competitor: Outdoorsy Hill Country** — Stonewall, 34 acres, **22 canvas tents**. Largest safari-tent inventory in the tracked set.
- **🆕 Two new earned-media surfaces** — **Yahoo News video feature** and **MTHR Collective**, joining Tribeza on brand queries. Three independent editorial surfaces is a real brand-entity signal. No action.
- **⚠️ WATCH — brand SERP ordering unstable again.** Bare `Rancho Moonrise` query put the owned site ~#9; a longer brand query put it ~#5 **with four additional owned pages** also surfacing. Same non-geolocated proxy artifact investigated and cleared 6/29. Not treated as a regression; a clean incognito or rank-tracker check would settle it permanently.
- **⚠️ Hotels.com — verification BLOCKED 4 consecutive runs.** Direct fetch of `ho2867109568` times out at 60s every time. Banned "20 luxury cabins / 50 guests" copy claim carried as `STALE:2026-06-29` — **not** confirmed and **not** resolved. Needs a manual read by Adam or an alternate path. See NEEDS ADAM.
- **Glamping Hub** — Rancho still absent, now **~18 weeks**. Free aggregator lever; close regardless of the on-site content play.
- **Walden Retreats** (15 tents, 96 ac, Johnson City) and **The Retreat on the Hill** (11 units, star-gazing dome) both surfaced but were not re-fetched — copy claims carried UNVERIFIED-on-page.
...
## 🚩 FLAG_FOR_ADAM — added 2026-08-17 by `rancho-competitive-weekly`

### `competitive-intelligence.html` has been ~4 months stale since April
Every Intel card on **both** improvement-plan dashboards links "View the full intelligence report →" to `/competitive-intelligence.html`. That page still reads **Research Date: April 6, 2026** (line 209) — it is a 752-line hand-maintained static page that was never wired to the weekly output. The live report lives in `site/competitive-intelligence.md`, rewritten every run. **Anyone who followed the dashboard link since April has been reading April data.** Needs a call: render the `.md` at that route, or retire the link. Not fixed this run — it is a build decision, not a competitive-intel edit.

### VOICE-GUIDE contradicts itself on "Manor" — and the Manor pages are the ones ranking
`VOICE-GUIDE.md:270` says the location descriptor is "20 minutes from downtown Austin" — **never "Manor"**; line 99 attributes the rule to Ashley. But line **230** of the same guide says to "naturally weave in location keywords (**Manor TX**, near Austin, Central Texas)." **30 site files** reference "Manor, TX", including three body-copy instances on `/blog/corporate-retreat-near-austin/` — **the page that just started ranking ~#4** — and an entire post `things-to-do-manor-tx.html` with "Manor TX" in its title, H1, meta and schema `keywords`. This is a deliberate SEO play against a stated brand rule, on the pages producing Rancho's only non-brand rankings. **Needs Ashley's ruling before anyone "fixes" it in either direction.** Flagged only — page edits belong to `rancho-site-daily` / `rancho-content-weekly`.

### A working SERP path exists — `rancho-seo-s7` (AEO baseline) may be unblocked
Today's `rancho-site-daily` run concluded (CHANGELOG 2026-08-17) that SERP/AEO measurement "cannot be measured from this client" after DuckDuckGo returned 202 challenge pages and Bing degraded into dictionary/Wikipedia results. **That conclusion is client-specific, not universal.** This task ran 8 live SERP queries today through a different path and got clean, coherent, cross-verifiable results — including the discovery that Rancho now ranks. **`rancho-seo-s7` should be retried using the path that worked here** rather than left marked unmeasurable. Worth reconciling between the two tasks.
