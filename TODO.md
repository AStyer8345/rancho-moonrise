...
Last updated: 2026-06-21 (rancho-site-daily — 22nd consecutive cruise-control firing, no shipped edits; Re-Verify Gate 7/7 still_true, 0 resolved (apex 200 + server: Vercel + x-vercel-cache: HIT age 60969; www 308 → apex; sitemap / corp-retreats / safari-tents / blog 200; BlogPosting CreativeWork 6-property spot-check intact 26 days post-5/26 deploy). Continuity fix: committed orphaned `run-logs/2026-06-20-seo.md` by explicit path. NEEDS ADAM #0 pause-list fix now **25 days overdue** (first surfaced 5/27). Run-log: `run-logs/2026-06-21-seo.md`. Prior header: 2026-06-20 rancho-site-daily — 21st consecutive cruise-control firing, no shipped edits; Re-Verify Gate 7/7 still_true, 0 resolved; commit by explicit path only.)
...
3. **GSC Request Indexing on the 2 uncrawled landing pages (~5 min).** `/corporate-retreats/` (~45d) + `/safari-tents-near-austin/` (~50d) are built and schema-complete but uncrawled — the schema work isn't cashing in until Google indexes them.
4. **ResortPass — activate or formally kill (~30s with Ashley).** 28+ day decision drift on a live-but-dormant zero-product listing. Even the no-product middle ground (publish hours, like Lucky Arrow) beats leaving it dormant.
...
## 📌 Competitive intel quick wins (refreshed 2026-06-15)

- **🆕 Broad SERP-layout shift toward aggregators/listicles across all glamping head terms.** Glamping Hub, Hipcamp, Teamout, VIP RE, Wilder, Texplorevibe surged to the top on `glamping near austin tx`, `safari tent austin texas`, `weekend getaway near austin glamping`. Branded #1-holders all slid beneath: Udoscape #1→~#6, Element Ranch #1→~#6 (Sage Hill to #1), Safari for the Soul #1→~#5. **Implication:** listicle/aggregator inclusion (Hipcamp curated, Glamping Hub) is now a more direct path to head-term visibility than on-site SEO — makes the 9-week Glamping Hub absence + Hipcamp curation gap more material.
- **🔄 Hipcamp curated "20 Best Glamping Near Austin" set ROTATED** — broke the 4-week byte-identical streak. New anchors (Urban Hideout #1, Cozy Cactus Airstream #2); **Lucky Arrow slipped #8→#9**; Rancho still absent (5th read). The 6/8 "zero rotation" was a one-week anomaly — auto-resolved to done-log. Reinforces Hipcamp Ashley question (NEEDS ADAM #3).
- **🆕 Corporate-retreat cluster churned again — The Yurtopian RE-ENTERED ~#3** after 4 weeks off. Sage Hill jumped to #1, Element Ranch fell ~#6; Lucky Arrow + Crystal Creek + Moniker Partners dropped off; Peaceful Waters + 7744 Ranch held. Camp Lucy still off. Cluster bottom-half reshuffles weekly while Rancho's `/corporate-retreats/` stays uncrawled (45d).
- **🔄 Spoon Mountain lost ground on both terms** — dropped off the safari-tent SERP entirely (was new ~#5 on 6/8) and slipped #1→~#5 on the narrow "romantic weekend getaways near austin" (displaced by travel-editorial: Visit Austin, VacationIdea, Yelp). Still the only glamping *property* on page 1 of the narrow term.
- **Rancho ResortPass listing EXISTS, dormant — decision drift now 28 days past 5/18 surface** ([resortpass.com/hotels/rancho-moonrise](https://www.resortpass.com/hotels/rancho-moonrise)). See NEEDS ADAM #2. Middle-ground: publish operating hours like Lucky Arrow, without committing products.
- **Lucky Arrow ResortPass dormant — hours unchanged 6/15** (daily Pool 12–10pm + Fri–Sun bar 2–6pm, 0 products). No movement toward products.
...
- **Walden Retreats** — 15 tents, copy still clean (no "four types" relapse). **Camp Lucy** corporate page still live + selling (off SERP).
- **⚠️ WATCH (UNVERIFIED) — brand SERP anomaly.** Non-localized WebSearch put ranchomoonrise.com ~#7 on its own brand term below Yelp/Do512/The Knot/Hipcamp/FB. Likely logged-out-SERP artifact; confirm via clean incognito/rank-tracker before any title-tag work.
- **⚠️ Hotels.com fetch blocked 6/15** — banned "20 cabins / 50 guests" copy still in live search index (6th wk); on-page UNVERIFIED this run. Needs browser-based check. See NEEDS ADAM.
