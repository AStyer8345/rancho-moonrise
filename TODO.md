...
Last updated: 2026-06-11 (rancho-site-daily — 12th consecutive cruise-control firing, no shipped edits; Re-Verify Gate 7/7 still_true, 0 resolved (apex 200 + server: Vercel + x-vercel-cache: HIT age 14746; www 308 → apex; sitemap / corp-retreats / safari-tents / blog 200; BlogPosting CreativeWork 6-property spot-check intact 16 days post-5/26 deploy). NEEDS ADAM #0 pause-list fix now **15 days overdue** (first surfaced 5/27). Run-log: `run-logs/2026-06-11-seo.md`. Prior header: 2026-06-08 rancho-competitive-weekly — weekly intel rewritten + dual-written; Re-Verify Gate 11 claims live (7 still_true, 2 CHANGED, 1 auto-resolved). Camp Lucy off corporate SERP after one week → done-log RESOLVED. Spoon Mountain reframed (#1 narrow / out broad). Hipcamp set unchanged. ResortPass drift 21d; corp-retreats 38d + safari-tents 43d uncrawled. Prior header below: 2026-06-06 rancho-site-daily — 7th cruise-control firing, no shipped edits. CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26; pre-scoped Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds noise not signal against a paused workstream). Re-Verify Gate clean (9/9 still_true, 0 resolved: apex 200 + server: Vercel + x-vercel-cache: HIT; www 308 → apex; sitemap 200; corp-retreats / safari-tents / blog 200; BlogPosting CreativeWork 6-property schema spot-check on `/blog/glamping-near-austin-texas/` intact 11 days post-5/26 deploy). NEEDS ADAM #0 30-second pause-list fix is now **10 days overdue** (first surfaced 5/27). Run-log: `run-logs/2026-06-06-seo.md`.)

## 🔥 NEEDS ADAM (highest leverage this week)

0. **NEEDS ADAM — Rancho scheduled tasks have now fired against paused workstream ELEVEN consecutive firings across 4 distinct task IDs (`rancho-content-weekly` 5/27 + `rancho-site-daily` 5/28 + `rancho-site-daily` 5/29 + `rancho-competitive-weekly` 6/1 + `rancho-review-monitor` 6/1 RUN_041 + `rancho-site-daily` 6/1 + `rancho-site-daily` 6/2 + `rancho-site-daily` 6/3 AM + `rancho-content-weekly` 6/3 PM + `rancho-review-monitor` 6/3 RUN_043 + `rancho-site-daily` 6/4 = 6th site-daily firing + 2nd content-weekly firing + 2 review-monitor firings + 1 competitive-weekly firing).** GOALS.md week of 2026-05-18 says "No Rancho Moonrise active work — paused (cruise control only if Ashley moves)"; none of these tasks are on the Keep-running list. Per CLAUDE.md rule ("Automated/scheduled tasks continue unless GOALS.md explicitly pauses them"), the spirit-of-the-pause says stop; the letter-of-the-pause does not explicitly name these tasks. **Pattern strengthens further with 6/5 + 6/6 `rancho-review-monitor` firings (RUN_045, RUN_046) and today's 6/6 `rancho-site-daily` firing — `rancho-site-daily` alone has now fired in maintenance mode 7 times in 9 days (5/28, 5/29, 6/1, 6/2, 6/3, 6/4, 6/6) — exactly the cadence the cruise-control gate is supposed to suppress.** Note on `rancho-review-monitor` specifically: produces no Adam-facing artifacts beyond carry-forwards already raised (Cassie + Haylee drafts, both surfaced for ~17 days now) and live state JSON; arguably the cleanest cruise-control candidate of the four. Decision needed: (a) add `rancho-content-weekly` + `rancho-site-daily` + `rancho-competitive-weekly` + `rancho-review-monitor` (plus any other rancho-* tasks Adam wants explicitly paused) to the GOALS.md Pause List so future fires exit cleanly, (b) remove them from the scheduled-task registry entirely, or (c) flip Rancho off "paused" if cruise-control work should keep running. The 30-second fix is option (a): one explicit pause-list bullet covers it. **The 30-second fix has now been on Adam's plate since 5/27 — 15 full days. Firing-count drift continues daily until Adam picks an option.** `rancho-site-daily` alone has now fired in cruise-control maintenance mode **12 consecutive times** (5/28, 5/29, 6/1, 6/2, 6/3, 6/4, 6/6, 6/7, 6/8, 6/9, 6/10, 6/11) — exactly the cadence the cruise-control gate is supposed to suppress. Second-order cost (surfaced 6/9, persists 6/10): the 6/8 site-daily run wrote its four-file updates + two run-logs to disk but the concurrent notebooklm-sync task committed over the top, so nothing landed — orphaned run-logs accumulate as a direct consequence of leaving these tasks live against a paused workstream (6/9 committed 3 orphaned SEO run-logs; the working tree still carries concurrent-writer `*" 2"`/`*" 3"` artifacts + `TODO.md.tmp.*` + `AGENTS*.md` + duplicated images that every run must now carefully avoid staging). `rancho-review-monitor` RUN_048 also fired 6/10. Next `rancho-content-weekly` fires ~6/10 unless paused.

1. **🆕 POST 2 REVIEW REPLIES — drafts ready in `brand/review-reports/2026-05-19-review-report.md`. DAY 24 UNPOSTED IN MONITOR (RUN_049, 2026-06-11).** (a) **Cassie Butterfield (Google 5★, ~26 days old, ~2026-05-16)** — corporate-retreat review, drafted reply is voice-clean and ready to paste as-is in GBP dashboard, 30 seconds. (b) **Haylee L. (The Knot 1★, 2026-02-26, ~105 days / ~15 weeks unreplied)** — quiet-property-risk complaint from a site visit, drafted reply has an edit-flag on the "we're flagging neighboring-property activity..." sentence — 2-minute decision on whether to publicly commit to that operational change before posting. **Visibility cost:** Haylee's verbatim review body still surfaces in Rancho-attributed snippets even on name-free queries — bad-review exposure persists, raising the cost of continued non-reply. **After posting, mark `rancho-review-replies-2026-05-20` done from the briefing page** so this monitor can stop carrying it forward.
2. **ResortPass listing exists — activate or formally kill (~30s with Ashley).** [resortpass.com/hotels/rancho-moonrise](https://www.resortpass.com/hotels/rancho-moonrise) is LIVE but dormant. **21-day decision drift since 5/18 surface.** Correct address (20117 Lockwood Rd), 3-star, pool hours 10am–6pm daily. Products section empty. Decision: "add day-pass products and activate" OR "leave dormant and remove from TODO." **6/8 context:** Lucky Arrow's dormant ResortPass listing now publishes a daily Pool window (12pm–10pm) on top of Fri–Sun bar hours (2–6pm) despite 0 products — the no-product middle ground (publish operating hours without committing products) is exactly what Rancho could do if undecided.
2. **GSC — request indexing on specific URLs (Priority 1, ~5 min).** `/corporate-retreats/` (**38 days uncrawled**, +7d from 6/1) + `/safari-tents-near-austin/` (**43 days uncrawled**, +7d). URL Inspection → Request Indexing for each. **6/8 update:** Camp Lucy dropped OFF the corporate-retreat SERP after exactly one week (page still live); cluster reshuffled to Peaceful Waters + 7744 Ranch. The pressure isn't any single entrant — it's that the cluster churns weekly while Rancho's page stays invisible. Crawl is the gate; both pages are fully built + schema-complete.
3. **Hipcamp strategy question for Ashley (carry-over from 2026-05-16 audit — REINFORCED 6/1).** Read `brand/2026-05-16-hipcamp-curation-gap-audit.md`. TL;DR: Rancho excluded from Hipcamp's Austin glamping editorial landing by 3 structural flags + 8 content gaps. **6/1 new development:** Lucky Arrow Retreat broke into Hipcamp's "20 Best Glamping Near Austin" at position 8 — first direct-competitor appearance in Hipcamp curated editorial set across 3 reads while Rancho stays out. If Hipcamp matters for bookings, visibility gap is now widening; if Hipcamp is backlink-only, gap is irrelevant. One question for Ashley: **"Is the Hipcamp listing intentionally private — SEO presence only — or do we want bookings from it?"** Two voice/data fixes worth doing regardless: acreage drift ("34-acre" vs VOICE-GUIDE's 36) + voice violation ("unwind with a drink at our bar").
...
6. **Glamping Hub submission.** 7 weeks running. Free, 15 min, glampinghub.com/list-your-property.

## 📌 Competitive intel quick wins (refreshed 2026-06-08)

- **Rancho Moonrise ResortPass listing EXISTS, dormant — decision drift now 21 days past 5/18 surface** ([resortpass.com/hotels/rancho-moonrise](https://www.resortpass.com/hotels/rancho-moonrise)). See NEEDS ADAM #2. Middle-ground option: publish operating hours like Lucky Arrow does, without committing products.
- **Lucky Arrow Retreat holds Hipcamp's curated "20 Best Glamping Near Austin" at position 8** (4 consecutive reads now). Rancho remains absent. Reinforces Hipcamp Ashley question (NEEDS ADAM #3).
- **✅ AUTO-RESOLVED — Camp Lucy OFF the corporate-retreat SERP after one week.** `camplucy.com/meetings-events/corporate-packages` page still live + selling packages, but dropped from the top 10 (one-week appearance). Logged `camp-lucy-corporate-serp — RESOLVED` to done-log. Same weekly-churn cadence as the rest of the corporate cluster.
- **🆕 Corporate-retreat cluster reshuffled — Peaceful Waters Retreat Center + 7744 Ranch in** (took the slots Camp Lucy + Texas Old Town vacated). Element Ranch (#1) + Crystal Creek hold. The Yurtopian off 4th consecutive week.
- **🆕 Glamping-near-austin reshuffled — Camposanto ATX + The Yurtopian dropped off; Green Acres Glamping new at #2** + a Texas Parks & Wildlife guide. Hipcamp listicle slid #1→#4 (Udoscape #1). Green Acres is now actually RANKING, not just a "track" item.
- **🔄 Spoon Mountain reframe** — weekend-getaway guide page OUT of broad term (2nd wk) but **#1 on narrow "romantic weekend getaways near austin."** Durable long-tail, not broad volume. Also NEW on the safari-tent SERP (~#5).
- **Safari for the Soul unit count genuinely ambiguous** — Sabi Nights page *titled* "Sabi Nights Yurt" but body copy says "our most luxurious safari tent." Honest read 1–2 safari tents + 3–4 yurts + 1 residential at `safariforthesoulglamping.com`. ±1 tolerance.
- **✅ Walden Retreats cleaned up** the "four accommodation types / three named" copy inconsistency — now cleanly three named variants; still 15 tents.
- **Lucky Arrow ResortPass dormant — now publishes daily Pool 12pm–10pm + Fri–Sun bar 2–6pm** despite 0 products. Continued activation-prep / no-product middle-ground pattern.
- **Hipcamp curated 20 unchanged from 6/1** — first non-rotating read since tracking began. Lucky Arrow #8, Rancho absent (4th read).
- **viprealestate.com** "Best Seven Glamping Getaways" + **texplorevibe** listicles hold weekend-getaway SERP — cluster keeps fragmenting toward listicles.
- **Google apex indexing — shallow footprint via filtered `site:` queries**; landing pages still uncrawled (38d + 43d, +7d drift each). See NEEDS ADAM #2 (GSC).
- **The Retreat on the Hill** — 11 named units; holds top-10 on safari-tent + weekend-getaway SERPs.
