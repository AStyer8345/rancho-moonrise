...
- **Update 2026-09-07 (RUN_073):** HTTP 429 a **5th time**, after a **17-day gap** since the last attempt (8/21) — longer than the 9-day gap that already disconfirmed the backoff hypothesis once. **Path (d) ("429 may self-heal") is now CLOSED per RUN_072's own stated threshold.** Remaining resolution paths are (a) and (c) only, plus the still-open rule question (b) already with Adam. A WebSearch this run surfaced "9.0 out of 10... 15 reviews" attributed in prose to Expedia — almost certainly the recurring Hotels.com-under-Expedia-label bleed documented since RUN_066 (the h89565924/ho2867109568 8.0/9.0 split family), not a genuine Expedia read. Logged as a new divergent value (15) in `search_snippet_divergent_values`; **not promoted**. 8.0 and null count carry unchanged.
