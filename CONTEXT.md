...
**Last updated:** 2026-04-28 (SEO daily — AggregateRating extended to 5 high-intent BlogPosting landing pages via publisher Organization)
...
- **Next SEO run (Apr 30+):** Today (04-28) extended AggregateRating to 5 high-commercial-intent BlogPosting landing pages (wedding-venues-near-austin, corporate-retreat-near-austin, ranch-wedding-texas, bachelorette-party-austin-texas, pool-day-pass-austin) — placed on `publisher.Organization` so the rating attaches to the venue, not the article. May 29 Free Friday is the next event to age out — rotate when it passes (next Free Friday already in Supabase `rancho_events`). Blog pipeline remains PAUSED per Ashley 2026-04-23. **Remaining 6 AggregateRating gaps:** glamping-near-austin, things-to-do-manor-tx, glamping-vs-camping, things-to-do-near-austin-with-kids (BlogPostings — same publisher pattern), and faqs.html + contact.html (utility pages — different schema, decide whether to add an Organization block or skip). Audit Review/AggregateRating numeric anchor (currently 125; review-monitor RUN_012 confirmed WebSearch snippet stable at 175 across 2 runs — wait for authoritative count from Adam or Places API key before bumping). Start drafting `/pages/corporate-retreats.html` if Adam greenlights (per competitive intel, Yurtopian/Lucky Arrow/7744 Ranch all have one).
...
- 2026-04-28 (review-monitor): **RUN_012. Google snippet now 2-run-stable at 175 reviews** (RUN_011 + RUN_012 both 175, vs prior 3-run-stable 126 in RUN_007–009 and Apr-9 baseline of 127). Two consecutive runs at 175 makes a one-time anomaly unlikely — likely real (~48 new reviews since April 9). Live GBP scrape still BLOCKER (12th — JS-rendered, no Places API key) so exact count and unreplied state remain unverifiable; FLAG_FOR_ADAM strengthened. No review-related done-log entries since RUN_011 — `rancho-review-replies` RESOLVED 2026-04-15 22:12 remains last review action; unreplied=0 maintained on done-log signal (caveat: implied ~48 new reviews may include unreplied ones). TripAdvisor live-verified 0/unclaimed. Hipcamp live-verified 0 ("Be the first to review", 1 booking, joined Mar 2024). Facebook 5/100% confirmed. Hotels.com 8.0 rating snippet surfaced cleanly (stronger than RUN_011); listing URL `ho2867109568` still active. The Knot detail expanded: WebSearch shows 4.5★ / 8 reviews (was 1 review snippet in RUN_008) — still not in scope, Adam may want to fold into review coverage. 3 BLOCKERS ongoing (Google run 12, Hotels.com timeout run 12, Airbnb 403 run 12). Dashboard status: ok. Adam: confirm GBP count via dashboard or provide Places API key for authoritative live counts.
