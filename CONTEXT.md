...
- **PRODUCTION IS PINNED to a CLI deploy — do not `vercel promote` a git build yet (2026-09-24).** A 9/23 CLI deploy by `howdy-4829` added `/offthegrid/` + an "Off The Grid" nav link (`js/main.js`) that are **not in git**. A git push on 9/24 wiped both for ~3 min; site-daily rolled back to `dpl_C479dkHeRwyReAvbyAcRQmuFjtbc`. Git pushes now build but do not go live. Fix: commit the Off The Grid source to `main`, then promote. Detail: `run-logs/2026-09-24-seo.md`.
