...
- **Next SEO run (Apr 27+):** April 26 Yoga & Mimosas (today's event, 11 AM start) passes tonight — rotate the static Event-schema entry + April-section card on the next run; next Yoga & Mimosas (May 31) is already in static HTML. Per CONTEXT 2026-04-23: May 29 + June 26 Free Fridays are already inserted into Supabase `rancho_events` — verify on next run if static fallback drift shows up. Blog pipeline remains PAUSED per Ashley 2026-04-23 (do not push blog #19). Schema/AEO sweep still has freshness work — next candidates: bump sitemap lastmod on pages that received AEO blocks since 2026-04-15 (currently stale), audit Review/AggregateRating coverage on landing pages.
