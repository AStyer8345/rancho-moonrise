...
**Last updated:** 2026-04-27 (SEO daily — April rotation, AggregateRating on weddings + host-your-event, sitemap freshness sweep)
...
- **Next SEO run (Apr 28+):** April 26 Yoga rotation done (today). May 29 Free Friday is the next event to age out — rotate when it passes (next Free Friday already in Supabase `rancho_events`). Blog pipeline remains PAUSED per Ashley 2026-04-23. Open candidates after this run: extend AggregateRating to remaining landing pages (faqs, contact, glamping/wedding/corporate/manor/bachelorette/ranch-wedding/pool/kids/glamping-vs-camping — 10 still missing); audit Review/AggregateRating numeric anchor (currently 125; review-monitor RUN_011 flagged WebSearch snippet at 175 — wait for authoritative count from Adam or Places API key before bumping); start drafting `/pages/corporate-retreats.html` if Adam greenlights (per competitive intel, Yurtopian/Lucky Arrow/7744 Ranch all have one).
