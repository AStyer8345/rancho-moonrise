...
- **🆕 NEEDS ADAM (GSC submit):** with DNS live and apex canonical settled, submit `https://ranchomoonrise.com/sitemap.xml` to Google Search Console. This is the rancho-seo-s6 task — was blocked on cutover, now actionable.
...
- **Next SEO run (May 1+):** Today closed the crawlability/canonical repair batch: apex primary, `www` permanent redirect, clean-route rewrites for all sitemap URLs, stale-claim scrub, asset-reference fixes, and `npm run validate:site`. Next priorities post-cutover: (1) GSC sitemap submission to apex, (2) `/pages/corporate-retreats.html` build (per competitive intel, Yurtopian/Lucky Arrow/7744 Ranch all have one), (3) AEO baseline measurement (rancho-seo-s7) — query AI engines for the 10 target keywords, baseline citation rate now that pages are crawlable. Leave static-HTML templating/partials refactor for a later branch; this repair batch intentionally stayed framework-free. May 29 Free Friday is the next event to age out — rotate when it passes (next Free Friday already in Supabase `rancho_events`). Blog pipeline remains PAUSED per Ashley 2026-04-23. Audit Review/AggregateRating numeric anchor stays 125 until Adam confirms the authoritative count or supplies a Places API key.
...
- 2026-05-01 (review-monitor RUN_015): **Google WebSearch snippet flipped back to 175 @ 4.9★** after RUN_014's 126 — snippet history now 126→175→126→175 across 15 runs (4-state ping-pong confirmed). Snippet is conclusively unreliable as authoritative count; rotates between at least two aggregator sources that disagree. FLAG_FOR_ADAM stays deescalated. TripAdvisor 0/unclaimed (price drifted $79-$180 → $77-$178, both -$2). Hipcamp 0 reviews. Facebook 5/100% confirmed. Hotels.com/Expedia 8.0 stable. 3 BLOCKERS ongoing (Google count, Hotels.com timeout, Airbnb 403) — all 15 runs. NEEDS ADAM: confirm GBP count via dashboard (60s) or supply Places API key (durable fix); 5 runs of asking, starting to look load-bearing.
