...
**Last updated:** 2026-09-26 (rancho-site-daily — **`/offthegrid/` hero went from a 3.0 MB PNG to a 342 KB WebP (−89%).** It's a CSS background, so the browser found it late. Added a `fetchpriority="high"` preload so the LCP image starts downloading with the HTML. The PNG stays in the repo until the live page is verified. Re-Verify Gate 6/6 still_true: brand canary passed, `/safari-tents-near-austin/` still absent from `site:` (9 owned URLs, owner GSC action), sitemap 32/32 200, dead `/videos/` card `_klefu2vTwM` (~day 65), `/host-your-event/` "1,000+ guests" x2, and the main checkout is still mid-rebase since 9/18 (worked from a worktree). `/offthegrid/` still not indexed (day 3; owner URL Inspection needed before Nov 14). Run-log: `run-logs/2026-09-26-seo.md`.)
...
**Prior entry:** 2026-09-25 (rancho-site-daily — **Made the new `/offthegrid/` event page (Off the Grid retreat, Nov 14) discoverable and shareable.** It was the only public page with no canonical, social tags, breadcrumb or sitemap entry, and its only internal link is JS-injected. Added canonical, OG/Twitter tags with a 1200×630 image cropped from its hero, `Event` + `BreadcrumbList` JSON-LD (no invented price), and a sitemap entry. Taught `validate-site.py` that static directory pages need no rewrite. Gate 5/5 still_true: safari-tents absent from `site:`, sitemap 31/31, dead `/videos/` card ~day 64, "1,000+ guests" still live, main checkout still mid-rebase (worked from a worktree). Production confirmed a git build before push. Full detail: `run-logs/2026-09-25-seo.md`.)

**Prior entry (2):** 2026-09-24 (rancho-site-daily — **No on-page defect found, none invented.** Re-Verify Gate 5/5 still_true: `/safari-tents-near-austin/` still absent from `site:` (owner GSC action), sitemap 31/31 200, dead `/videos/` card `_klefu2vTwM` (~day 63), `/host-your-event/` "1,000+ guests" still live, main checkout still mid-rebase (worked from a worktree off `origin/main`). First-ever image-dimension (CLS) and live canonical/robots sweeps: both clean. Closed 5 stale TODO items after live checks. Full detail: `run-logs/2026-09-24-seo.md`.)
...
- 2026-09-25 — review-monitor RUN_080: **no new reviews anywhere.** Airbnb aggregates held on a 2nd direct fetch (3.67/3 · 4.71/7 · 4.0/1, host 15 @ 4.47); safari-tent listing retitled. Airbnb review text still unreadable (`airbnb-review-text` WATCH 2 of 3; the in-app browser attempt timed out), so the two 3★ reviews still need a human look. New surface: Swimply pool listing indexed but its page 404s (possibly delisted, out of scope). Hotels.com 9.0/14 seen a 2nd time, not promoted. Haylee L. unreplied day 211; 2 drafts unposted day 129. Detail: `tasks/review-monitor/session-log.md`.
