...
- 2026-04-25 (`/api/inquiry` decision reversed): **Rolled back the morning's Resend pivot — `/api/inquiry` is back to POSTing the CRM directly.** The notification-to-events@ behavior moved to the n8n draft-reply workflow on the CRM side (new "Build Notification Email" + "Send Inquiry Notification" branch). End state: form → CRM webhook (canonical) → Supabase INSERT → trigger fires draft-reply workflow → AI draft + notification email both arrive in events@. Ashley's inbox still looks like today's WordPress pattern. No Resend/DKIM setup needed on the marketing side. `.env.example` retained (now without the Resend block) since the existing `RANCHO_CRM_WEBHOOK_URL` / `RANCHO_API_KEY` / `BRIEFING_AUTH_TOKEN` / `GITHUB_TOKEN` env vars are still useful documentation.
