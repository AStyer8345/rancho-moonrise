...
- 2026-04-27 (review-monitor): **RUN_011.** Google WebSearch snippet jumped 126 → 175 reviews — first count change since RUN_007 broke a 3-run-stable pattern. Snippet precision insufficient as authoritative count, but +49 jump too large to dismiss — logged as new FLAG_FOR_ADAM. Live GBP scrape still BLOCKER (JS-rendered, no Places API key); cannot confirm exact count or whether new reviews are unreplied. No review-related done-log entries since RUN_010, so unreplied=0 maintained on done-log signal (`rancho-review-replies` RESOLVED 2026-04-15 22:12 still last review action). TripAdvisor live-verified 0 reviews/unclaimed. Hipcamp live-verified 0 reviews ("Be the first to review", 1 booking no review, joined Mar 2024). Facebook 5/100% confirmed. Hotels.com listing still active per WebSearch but rating snippet weak this run. 3 BLOCKERS ongoing (Google JS-blocked run 11, Hotels.com timeout run 11, Airbnb 403 run 11). Dashboard status: ok. Adam: confirm GBP count via dashboard or provide Places API key for authoritative live counts.
