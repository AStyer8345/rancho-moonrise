...
## 2026-08-19 (rancho-site-daily) — entity graph consolidated (`sameAs` 4 → 11); banned third-party copy escalates STALE → CONFIRMED and is not Hotels.com's alone

- **The site had exactly one `sameAs` array sitewide** — `site/index.html:93`, on the root `LodgingBusiness`/`EventVenue` node — carrying **four social profiles and zero review, booking or OTA profiles.** ResortPass (53 reviews, Rancho's 2nd-largest pool after Google), Hipcamp, The Knot, TripAdvisor, Yelp and Hotels.com all sat **outside the declared entity graph**. An engine reconciling "what is Rancho Moonrise" found the owned domain as one peer among many rather than the hub those listings hang off.
- **Shipped: 7 URLs added, `sameAs` 4 → 11.** YouTube `@ranchomoonrise` (direct 200), ResortPass (direct 200), Hipcamp (direct 200 + search), The Knot, TripAdvisor, Yelp, Hotels.com (all search-confirmed; all 403/429 to direct fetch — a bot-block, not an absence, per the 8/17–8/18 curl lesson).
- **5 candidates deliberately excluded, each with a reason.** A wrong `sameAs` asserts a false identity into the graph and is worse than a missing one. **Expedia** (`h89565924`) — 429 direct, and the search pass returned an unrelated *"Manor Condominium Resort"* page instead of the listing; in-repo but **not confirmed today**. **WeddingWire** — a URL exists in the repo while `TODO.md`'s 2026-06-18 audit states no listing exists; the repo contradicts itself. **Airbnb** — the recorded URL is a room URL, not an entity profile. **Roadtrippers** — third-party directory, not owned. **Google Business Profile — no canonical `g.page`/place-id URL exists anywhere in the repo**, which makes the largest review pool on the property (~130) the single highest-value missing entry. New NEEDS ADAM, one line, ~30 seconds.
- **Reported without overselling: `sameAs` is not a ranking factor and this edit will not move a single position.** It is a knowledge-graph / AEO signal. It is the correct *category* now that on-page levers are measured dry and AI-engine citation coverage (4/10) is a tracked metric — but it is slow and indirect, and the run-log says so in those words.
- **Tension recorded rather than hidden:** two of the added URLs (The Knot, Hotels.com) are the surfaces carrying the banned copy below. `sameAs` asserts identity, not endorsement — and asserting identity is what makes the owned domain's description authoritative in a reconciliation. Omitting them would leave the graph incomplete without removing one instance of the bad copy.
- **`hotels-com-banned-copy` escalates from `STALE:2026-06-29` to CONFIRMED — and is mis-scoped.** The claim has been unverifiable by direct fetch **4 consecutive runs** (60s timeout every time). Today the sentence *"Rancho Moonrise contains 20 luxury cabins and safari tents for up to 50 guests"* surfaced verbatim across **two independent queries**, attributed to **The Knot marketplace listing** — not Hotels.com. It is a **syndicated venue description live on at least two platforms**, and it carries three `VOICE-GUIDE.md` violations in one sentence: the banned word **"luxury"**, a **banned specific unit count**, and **"up to 50 guests"** — which contradicts both the site's own `maximumAttendeeCapacity: 200` and The Knot's *own* page claiming **300+**. The listing contradicts the site and itself.
- **Site-side re-verified clean:** `grep -ril 'luxur'` and `grep -ril 'hill country'` across `index.html` + all 29 pages → **0 matches**. The violation is entirely off-domain. Nothing in this repo to fix; the fix is a listing edit only Adam or Ashley can make.
- **A third on-page lever tested and disconfirmed: exact-match title/H1 — and it fails backwards.** `wedding-venues-near-austin.html` carries the exact phrase "Unique Wedding Venues Near Austin TX" in its `<title>` and **does not rank** for `unique wedding venues near Austin`; the same page holds the **best position in the entire 10-keyword set (4/8)** on `overnight event venue Austin`, a phrase **absent from its title**. `pool-day-pass-austin`, `things-to-do-manor-tx` and `glamping-near-austin-texas` all carry exact-match titles and rank nowhere.
- **Three levers now measured and disconfirmed** — internal-link depth (8/18, inverse), word count + schema depth (8/18, uniform), title/H1 match (8/19, backwards). **What ranking actually tracks is SERP fragmentation:** Rancho ranks where the competition is `ask.metafilter.com` / `premierpartycruises.com` / `fearlesscaptivations.com`, and is absent wherever aggregators consolidate — Glamping Hub, Hipcamp, Booking, Cvent, EventUp, Eventbrite.
- **Strategic consequence, and it is uncomfortable: on aggregator-owned head terms on-page work cannot win, because the winners are platforms, not pages.** The lever for those terms is *being in the aggregator*. That promotes the two cheapest and longest-ignored items in `TODO.md` — **Glamping Hub submission (free, 15 min, ~18 weeks untouched)** and the **Hipcamp curation gap** — above every remaining on-site edit. A reprioritization, not a new task.
- **Validation.** All 3 `index.html` JSON-LD blocks parse-clean via `python3 json.loads`; `sameAs` asserted at 11 entries, 0 duplicates, all `https://`; `npm run validate:site` passes; sitemap `<lastmod>` for `/` synced 2026-05-06 → 2026-08-19. Live health: 6 routes + sitemap all 200, TTFB 0.189–0.324s.
- **Diff:** 2 files, +7/−1. Schema-only — no copy changed, no visible change. Pre-existing prior-session modifications in `api/inquiry.js`, `site/css/styles.css`, `site/js/main.js` and 5 page files intentionally **not** staged (5/7– convention).
- **Re-Verify Gate: 8 claims — 6 still_true, 1 escalated, 1 fixed-in-run, 0 resolved.** Skipped per SKILL ownership rules: The Knot review count surfaced today as *7 @ 5★* vs the repo's *8 @ 4.5★* — review counts and reply state belong to `rancho-review-monitor`.
- **Flagged, not actioned:** ~20 untracked file-sync conflict duplicates (`CONTEXT 2.md`, `TODO 4.md`, `weddings 2.html`, `styles 2.css`, …) plus a stray `TODO.md.tmp.87098.…`. Untracked so they do not deploy, but two sit inside the deploy root and every grep in this repo now has to filter them out. Deleting files is not this task's call — recommend a `.gitignore` line or a manual sweep.
- Run-log: `run-logs/2026-08-19-seo.md`.

## 2026-08-18 (rancho-site-daily) — `rancho-seo-s7` MEASURED and RESOLVED: 4/10 keywords ranking, 4/10 answer-engine citations; yesterday's "unmeasurable" verdict overturned in one query

- **`rancho-seo-s7-aeo-baseline` is baselined — 4/10 target keywords ranking, 4/10 answer-engine citations.** Pending since 2026-05-04. Written to `tasks/seo-aeo/serp-baseline-2026-08-18.md` with the full 10-keyword table, top-5 competitor domains per keyword, pattern analysis, explicit methodology limits and monthly re-run instructions.
- **The verdict from 24 hours earlier was wrong, and how it was wrong is the durable finding: bare `curl` is bot-detected; the harness `WebSearch` tool is not.** Yesterday's run did the hard part right — it generated a clean-looking "0/10 ranking" table, recognised that it matched the known prior state exactly, and killed it rather than shipping it. Then it over-generalised from "this script cannot measure" to "this client cannot measure," and opened a Priority-1 API-key ask on that basis.
- **CONTEXT.md had already flagged the contradiction and asked for reconciliation before anyone bought a key** — `rancho-competitive-weekly` ran 8 successful SERP queries the same morning `rancho-site-daily` declared measurement impossible. **Reconciled: one variable, and it was not the credential.** The competitive task used the harness tool; the site task shelled out to `curl` from `scripts/serp-baseline.py`.
- **Brand canary passed on the harness path before any absence was recorded.** `Rancho Moonrise` returned `ranchomoonrise.com` alongside Yelp, Do512, Hotels.com, The Knot, LinkedIn, Facebook and Expedia. The same canary on the curl path the day before returned `gorancho.com`, `merriam-webster.com`, `miranchogrill.com` — and not the site. The guard did its job in both directions.
- **Ranking URLs:** `/weddings/` on `wedding venue Austin TX ranch` — **the first core landing page ever recorded ranking on this property**; `/blog/corporate-retreat-near-austin/` on `corporate retreat venue Austin TX`; `/blog/bachelorette-party-austin-texas/` at 5/9 (**new to any report**); `/blog/wedding-venues-near-austin/` at 4/8 (**new, and the best position in the entire set**).
- **3 of the 4 rankings are blog URLs — and the blog pipeline has been PAUSED since 2026-04-23.** The pause is Ashley's call and stays hers, but it has never been made with this number in front of her.
- **Rancho wins specific intent and loses every head term to aggregators** — Glamping Hub, Hipcamp, Booking, EventUp, Cvent, Eventbrite, The Knot, ResortPass. Sharpest instance: **ResortPass holds a top-5 slot on `pool day pass Austin TX` while Rancho sells day passes on ResortPass.** Rancho reaches that SERP only as a tenant on someone else's domain, while its own `/pool-day-pass-austin/` page ranks nowhere.
- **The queued "deepen internal links 4 → 5" pass was retired rather than performed.** Measured across the cluster, the correlation is *inverse*: the two best-ranking pages have the **fewest** inbound links (5 each), while the most-linked page (11 inbound) does not rank at all. Word count (1,572–2,222) and schema depth (4–5 JSON-LD blocks) are uniform across ranking and non-ranking pages alike. **On-page and internal-linking levers on this cluster are exhausted — now measured rather than asserted.** CONTEXT had queued this as "mechanical, low-risk, always available"; it is all three, which is exactly why it kept getting queued, and running it would have manufactured the appearance of progress.
- **Concrete fix shipped — a wrong entity fact in the instruction path that generates content.** `tasks/seo-aeo/master-agent.md:51` and `backlog.md:48` — the AEO entity-authority template, i.e. the exact sentence agents are told to propagate into copy — both read **"34-acre"**. That contradicts `VOICE-GUIDE.md:271` ("36 acres (not 20, not 31)") and all **183** acreage instances across 31 site files. Corrected to **36**. The contested "Manor, TX" half of the same sentence was deliberately left untouched pending Ashley's ruling (`VOICE-GUIDE.md:270` bans it, line 230 instructs it).
- **Site-side acreage drift claim auto-resolved.** Full audit returned 183/183 instances reading 36, zero at 34 or 37. The drift carried in CONTEXT belongs to the third-party **Hipcamp** listing, not to ranchomoonrise.com — nobody should "fix" the site.
- **Third-party listing copy is winning the brand entity, and it is copy the brand bans.** The synthesized brand answer described the property as *"20 luxury cabins and safari tents for up to 200 guests."* `VOICE-GUIDE.md` bans **"luxury"** and bans citing a **specific unit count**. This is the `hotels-com-banned-copy` claim `rancho-competitive-weekly` has failed to verify by direct fetch **four consecutive runs** (60s timeout). Still unconfirmed *on-page* — but now confirmed *in propagation*.
- **Cross-repo fix:** `client-ops/templates/re-verify-before-report.md` now names the harness search tool as *the* SERP verification path for every client task, placed ahead of the known-dead bare-curl table, with the natural experiment written up as the justification.
- **`tasks/seo-aeo/BLOCKERS.md`** — SERP-measurement blocker moved to Resolved one day after opening; GSC crawl gate **narrowed from two URLs to one** (`/corporate-retreats/` dropped after riding 8 consecutive reports, indexing confirmed twice by two tasks); new coverage-gap note recording that AEO citation is measured on **one** engine. Duplicate `## Resolved` heading fixed.
- **`scripts/serp-baseline.py`** — retained with its canary guard intact; docstring now warns that the curl path produces a false negative and points at the harness tool until an API key lands.
- **The API-key ask is downgraded from Priority-1 blocker to nice-to-have.** `rancho-seo-s7` is baselined without it and SERP verification works fleet-wide without it. Still worth having for certified absolute ranks and unattended script runs — but **nothing is waiting on it**.
- **Health:** 6 live routes + sitemap all **200**; TTFB 0.245–0.404s (no regression vs 0.19–0.44s); `npm run validate:site` **passes**. Dead YouTube video `_klefu2vTwM` on `/videos/` re-verified **403 — day 26**.
- **Re-Verify Gate: 8 claims — 3 still_true, 1 second-independent-confirmation, 4 resolved.**
- **No site HTML changed.** Today's defect was in the instruction path that generates content, not in the content. Prior-session uncommitted changes (`api/inquiry.js`, `styles.css`, `main.js`, `blog.html`, `contact.html`, `videos.html`, `wedding-venues-near-austin.html`, `weddings.html`) intentionally **not staged** — standing convention since 5/7.
- Run-log: `run-logs/2026-08-18-seo.md`.

## 2026-08-17 (rancho-review-monitor) — RUN_068: 34-run quiet streak BROKEN; ResortPass found carrying 53 unmonitored reviews; Facebook goes negative for the first time in 48 runs

- **rancho-review-monitor: scraped 8 platforms, 13 claims through the Re-Verify Gate, 0 stale claims resolved, 0 response drafts written (see Facebook — that is the correct outcome, not a gap). Status → URGENT.** 25-day coverage gap; task did not fire 7/24–8/16.
- **ResortPass carries 53 reviews at 4.8★ and is not in monitored scope.** +8 reviews and −0.1★ since 2026-07-15 (45 @ 4.9★). Verified by **two independent direct fetches the same morning** — this task's own and `rancho-competitive-weekly`'s — returning identical values, which is what made it safe to record rather than a single-source snippet. ResortPass appears nowhere in `master-agent.md`'s ownership table, nowhere in `review-aggregate.json`, and nowhere under `brand/` or `tasks/review-monitor/`. At 53 reviews it is Rancho's **second-largest review pool after Google — four times The Knot (8) and Facebook (6) combined.**
- **The structural finding: `resortpass.com` returns HTTP 200 to a plain fetch and parses cleanly, while six of the eight in-scope platforms are hard-blocked** (Google JS/API-gated, Airbnb 403, Hotels.com timeout, The Knot timeout, Hipcamp render-fail, TripAdvisor 403/domain-block). The one review surface that is actually scrapeable is the one nobody was scraping. Reply coverage recorded as **`null`, not zero** — there is no per-review enumeration path (`/hotels/rancho-moonrise/reviews` returns HTTP 404), so "unknown" is the honest value and "clean" would have been a fabrication.
- **Facebook moved for the first time in 48 consecutive runs, and it moved negative.** 5 → 6 reviews, 100% → **86% recommend**. Confirmed across **three independent queries before recording**, precisely because the prior value had 48 runs of stability behind it and a single snippet would not have been trustworthy. A drop from 100% means **at least one non-recommend now exists where there were zero.**
- **No response draft was written for it, deliberately.** Facebook's page is JS-gated — a direct fetch returns the page title and nothing else — and no query surfaced the review body. **The review text could not be obtained, so drafting a reply would have meant inventing what the guest said.** A fabricated draft in Ashley's voice answering a complaint nobody has read is worse than an empty slot. 60 seconds on the FB Reviews tab unblocks a real draft next run.
- **Arithmetic caveat recorded rather than smoothed over:** 5 of 6 is 83.3%, not 86% (6 of 7 is 85.7%). The displayed percentage does not divide cleanly into 6 reviews. Both figures are stored **as-displayed** rather than reconciled to whichever guess looked tidier. The direction is not in doubt; the exact composition is.
- **Hotels.com: new dated review 2026-07-11** — "The pool was super well kept and pretty chill…" — positive, above the ≤4★ draft threshold, no draft required. **The OTA split holds a third consecutive run and gains a third value:** expedia `h89565924` → **8.0**, hotels.com `ho2867109568` → **9.0 "Wonderful"**, agoda `h82700060` → **8.6**, the last now carrying its own distinct review quote. Three ratings on three OTA entities for one property. The 8.0 anchor was **not** overwritten — no fresh direct scrape is possible (43-run BLOCKER).
- **Everything else held.** Google snippet oscillated 175 → **126** (has now printed 126, 130 **and** 175 — not a proxy in either direction); live-authoritative 130 is now **90 days stale**. The Knot: Haylee L. 1★ still live, body still indexed verbatim, **no owner response anywhere — 172 days / ~24.6 weeks unreplied**. TripAdvisor 0/unclaimed held. Hipcamp 0 held, with "34-acre" and "a bar" voice violations **re-confirmed live**. Airbnb 403, 67th no-attempt run.
- **Two drafts still unposted at day 90** (Cassie Butterfield Google 5★, Haylee L. The Knot 1★, both drafted RUN_034). Re-verified against the operative done-log at repo root: last review-reply resolution is still **2026-04-15**, no Cassie entry, no Haylee entry. Genuinely unposted.
- **Re-Verify Gate: 13 claims — 10 still_true, 2 new_signal, 1 unknown, 1 verification-failure, 0 resolved.** The gate earned its keep in the opposite direction from usual: rather than killing a stale claim, it (1) forced three confirmations before a 48-run-stable value was overwritten, and (2) produced "no draft" instead of a plausible-sounding invention.
- **Scope change recommended but NOT executed:** adding ResortPass to `master-agent.md`'s live-claim ownership table is a scope decision, not a data write, so it was logged to TODO.md rather than made unilaterally.
- **3 FLAG_FOR_ADAM:** ResortPass reply coverage (~2 min in host dashboard); the unread negative Facebook review (60 sec); the three-entity OTA split. Run-log: `run-logs/2026-08-17-review-monitor.md`.

## 2026-08-17 (rancho-competitive-weekly) — Rancho ranks on non-brand keywords for the first time; `/corporate-retreats/` indexing blocker RESOLVED; two prior claims corrected

- **First non-brand rankings in the history of this report.** Every prior run since April opened with "0/4 non-brand keywords." Three owned URLs now surface on commercial queries, confirmed across three independent live queries plus a `site:` check: `/blog/corporate-retreat-near-austin/` ~**#4** on `corporate retreat near austin texas` **and** ~#4 on `corporate retreat venue near austin ranch`; `/corporate-retreats/` ~**#6** on the ranch variant; `/blog/weekend-getaways-near-austin/` ~**#5** on `weekend getaway near austin glamping` and ~**#4** on `best weekend getaways near austin texas 2026`.
- **`/corporate-retreats/` is INDEXED — the #1 Adam-side GSC action across 8 consecutive reports is closed.** A `site:ranchomoonrise.com` query returns it as the top result. The ~105-day uncrawled claim is dead. **The GSC ask narrows from two URLs to one:** `/safari-tents-near-austin/` remains uncrawled at ~113 days (+33d drift) and is now the lone holdout — a targeted `site:` on that exact path returned `/accommodations/`, `/host-your-event/`, `/weddings/` and blog URLs, but never the safari-tents page.
- **The blog post outranks the purpose-built landing page.** ~#4 vs ~#6 on the same query family; weekend-getaway is carried entirely by a blog URL with no landing-page support. Both rankings appeared **while the blog pipeline has been paused since 2026-04-23**. The "publish own-domain guide/listicle content" lever this report has recommended since May — modeled on Spoon Mountain and Cameron Ranch — is now demonstrated on this property rather than inferred from competitors. The months of schema/AEO enrichment (`wordCount`, `articleSection`, `ImageObject`, `speakable`, FAQPage) landed on exactly the pages now ranking.
- **Re-Verify Gate caught two false prior-report claims before republication — its clearest return to date.** (1) **Rancho's ResortPass listing does not sell four products.** The 7/15 card recorded Half-Day $15, Day Pass $20, Pilates and Horses $58, Full Moon Floating Sound Bath $30 at 4.9★/45 reviews. Live: **2 products only** (Half-Day from $15, Day Pass from $20) — **both experience products are gone** — at **4.8★ / 53 reviews**. Without the gate this report would have re-published a revenue surface that does not exist. Both removed products map to known recurring ranch programming, so it reads as a lapse rather than a delisting; nothing is logged in-repo either way → Ashley question. (2) **Lucky Arrow raised day-pass pricing 17%** — Adult and Family Pool Day Pass both $30 → **$35**; other four products and all hours unchanged; 4.6★ / 200 reviews.
- **Pricing headroom quantified for the first time.** Rancho: $20 day pass / $15 half-day at **4.8★ across 53 reviews**. Lucky Arrow: **$35** at **4.6★ across 200 reviews**. Rancho is **43% below** the nearest comparable at a *higher* rating, against a competitor that just demonstrated the market absorbs an increase. Revenue lever requiring a decision, not a build.
- **Three new corporate-retreat entrants in the same cycle Rancho entered the cluster.** **Serana** (Paige TX, 8 units — 5 Post Oak Cabins / 3 Kampinas / 1 Field Cabin — **$1,000/night flat full-property weekday**, Starlink + co-working lodge, sauna, cold plunge, salt-water pool) at ~#3 is the closest analogue to Rancho's buyout model **and publishes a flat price where Rancho custom-quotes**. **7744 Ranch** markets "just east of Austin — approximately 20 minutes from downtown," **Rancho's exact geographic hook**, at 100 guests / 10 overnight vs Rancho's 200 / full-ranch lodging. **Artemis Ranch** also entered. New content gap logged: published buyout pricing.
- **Three more claims auto-resolved.** **Spoon Mountain price inconsistency** — page rebuilt as a **10-property roundup** (new H1 "10 Seasonal Picks (2026 Guide)"); no `$245` figure anywhere; the competitor upgraded to the exact page format this report recommends Rancho adopt. **Safari for the Soul unit-count ambiguity** — settled at **5 glamping units** (Marble Falls 4, Spicewood 1) plus **1 separate residential** (KickBack Cove Home); the long-carried "6" was double-counting the house. Both written to `rancho-done-log.md`.
- **Other movement.** Hipcamp curated set **re-stabilized** (top 8 byte-identical to 7/15, Lucky Arrow holds #8); Rancho absent **8th consecutive read**. Glamping head term — **aggregators retook the top 3** (Outdoorsy #1, Glamping Hub #2, Hipcamp #3), Talula Mesa slid #1 → ~#4 (confirmed 4 tents / 15 acres / Marble Falls). Lucky Arrow **re-entered the corporate SERP ~#2** (was off the top 10). New safari-tent competitor **Outdoorsy Hill Country** — Stonewall, 34 acres, **22 canvas tents**, the largest inventory in the tracked set. Glamping Hub absence now **~18 weeks**. Two new earned-media surfaces — **Yahoo News video** and **MTHR Collective** — joining Tribeza.
- **Two verification paths remain unresolved.** **Hotels.com blocked a 4th consecutive run** (60s fetch timeout) — the banned "20 luxury cabins / 50 guests" claim is carried as `STALE:2026-06-29`, neither confirmed nor resolved, per the gate. **Brand-SERP ordering instability recurred** — bare query put the owned site ~#9, a longer brand query put it ~#5 with four additional owned pages also surfacing; consistent with the non-geolocated proxy artifact cleared on 6/29, so carried as WATCH rather than a regression.
- **Three FLAG_FOR_ADAM items raised.** (1) **`competitive-intelligence.html` has been ~4 months stale** — every Intel card on both dashboards links to it, and it still reads "Research Date: April 6, 2026"; the live report has lived in the `.md` all along. (2) **`VOICE-GUIDE.md` contradicts itself on "Manor"** — line 270 bans it as a location descriptor, line 230 instructs weaving it in as a keyword; 30 site files use it, including three body-copy instances on the page that just started ranking and an entire Manor-targeted post. Needs Ashley's ruling before anyone "fixes" it. (3) **A working SERP path exists** — today's `rancho-site-daily` run concluded AEO/SERP measurement is impossible from its client after DuckDuckGo and Bing both failed; this task ran 8 clean cross-verifiable SERP queries through a different path, so `rancho-seo-s7` should be retried rather than left marked unmeasurable.
- Intel card dual-written to both improvement-plan files, verified byte-identical. Full report rewritten at `site/competitive-intelligence.md`. Run-log: `run-logs/2026-08-17-competitive.md`.

## 2026-08-17 (rancho-site-daily) — SERP/AEO measurement proven blocked; harness built with a canary that refuses to fabricate a baseline

> ⚠️ **CONCLUSION OVERTURNED 2026-08-18.** The "unmeasurable from this client" verdict below was over-generalized from one failing path. Bare `curl` is bot-detected; the harness `WebSearch` tool is not. The same 10 keywords through the harness produced a real baseline the next morning — **4/10 ranking** (`tasks/seo-aeo/serp-baseline-2026-08-18.md`). **What still stands:** the false "0/10" table really was caught and killed, the canary really did fail on the curl path, and the bare-curl DDG/Bing findings below are accurate for those paths. Read the rest as a correct diagnosis of `scripts/serp-baseline.py`, not of the client.

- **Attempted the #1 queued slot (`rancho-seo-s7` AEO/SERP baseline) and proved it cannot be measured from this client.** The slot has sat "pending" since 2026-05-04 without anyone testing whether it was *runnable*. It isn't. Every available SERP path is dead or unreliable: `html.duckduckgo.com/html/` and `lite.duckduckgo.com/lite/` both return **HTTP 202 challenge pages with 0 organic results**; `bing.com/search` returns 200 and parses cleanly but **degrades after a handful of automated queries** into a generic dictionary/Wikipedia result set.
- **A false "0/10 ranking" finding was caught and killed before it reached CONTEXT.md.** A full 10-keyword Bing run produced a clean, well-formed table reading *"0/10 target keywords ranking"* — which **matches the known prior state exactly** and would have read as confirmation. It was pure artifact. Tells: `corporate retreat venue Austin TX` → stackoverflow.com ×5; `unique wedding venues near Austin` → merriam-webster.com + baike.baidu.com; `things to do Manor TX` → culturedcode.com (the Things app); `events venue Austin TX` → **Houston** results.
- **The brand canary settled it.** Querying `Rancho Moonrise` — a term the site holds at **#1** (confirmed 2026-06-29) — returned `gorancho.com`, `en.wikipedia.org`, `merriam-webster.com`, `miranchogrill.com`, and **not** `ranchomoonrise.com`. The engine could not see the site on **its own brand term**, so no "absent" reading from that session meant anything.
- **Shipped `scripts/serp-baseline.py`** — 10-keyword baseline harness, keyword list mirroring `tasks/seo-aeo/master-agent.md`. Three design choices, each a bug hit and fixed during this run: (1) **parse by opening marker, not balanced tags** — Bing nests `<li>` inside `b_algo` blocks, so `.*?</li>` truncates (first attempt: 10 blocks, 0 titles); (2) **domain from `<cite>`, not the `<h2>` href** — that href is a bing.com tracking redirect, so reading it reported `bing.com` as the domain of all 50 results; (3) **brand canary as a hard gate** — on failure the script writes `MEASUREMENT BLOCKED` with **zero** per-keyword rows and exits 2, making it structurally incapable of emitting a false "0/10". Pins `mkt=en-US&cc=US` for run-to-run comparability (an unpinned query drifted to Homewood, AL).
- **`tasks/seo-aeo/BLOCKERS.md` rewritten — stale DNS blocker auto-resolved after 108 days.** The file still read "DNS cutover not complete — all SEO/AEO work is prep-only until this is resolved," gating the entire workstream on a condition resolved **2026-04-30**. Live re-verification: apex 200 + `server: Vercel` + cache HIT, www 308 → apex, 9/9 routes 200. Moved to Resolved with evidence; real current blockers (SERP measurement, GBP access, GSC crawl gate) now listed.
- **Cross-repo fix — `client-ops/templates/re-verify-before-report.md`.** The shared fleet runbook named "DuckDuckGo HTML" as *the* verification path for SERP-position claims. That path is dead, meaning **every client task following the runbook has been unable to verify a SERP claim it surfaces.** Added a "Known-dead verification paths" table and made the brand canary a mandatory fleet-wide guard before any "target is absent from SERP" finding may be recorded.
- **Re-Verify Gate:** 6 claims — 5 still_true (apex/www/9 routes/dead-video day 25/no speed regression), **1 resolved** (DNS blocker). TTFB 0.19–0.44s across 4 pages, no CWV regression. `npm run validate:site` passes.
- **False alarm caught:** `/glamping-near-austin-texas/` 404s, but the canonical is `/blog/glamping-near-austin-texas/` (200, correctly in sitemap) and `grep` found **0** internal links to the bare path — my URL guess was wrong, not the site. Same shape as yesterday's `pool-day-pass-austin` false alarm; the missing `/blog/` prefix is the recurring trap.
- **No site HTML changed.** Tooling, blockers and docs only. **FLAG_FOR_ADAM:** (1) one search API key unblocks `rancho-seo-s7` and restores SERP verification fleet-wide — Google Programmable Search Engine is free at 100 q/day, ~10 min to wire; (2) dead YouTube video on `/videos/` now **day 25**. Run-log: `run-logs/2026-08-17-seo.md`.

## 2026-08-16 (rancho-site-daily) — Schema workstream closed at 100%; blog-cluster link floor restored to uniform 4

- **23-day coverage gap.** No site-daily run between 2026-07-24 and today. The last SEO run-log (`2026-07-23-seo.md`) was itself never committed; the only repo activity in the window was `notebooklm-sync` commits, most auth-blocked. Not reconstructable. The orphaned 7/23 run-log is committed with this run.
- **The entire pre-scoped "next slot" queue was already shipped.** CONTEXT.md's #1 slot ("`videos.html` carries no JSON-LD at all") and its #2 ("`accessibility.html` + `policies.html` lack WebPage/speakable") were both closed by commits that landed *after* the last CONTEXT.md rewrite — `089439f` (7/23, videos: CollectionPage + BreadcrumbList + ItemList of 3 VideoObject) and `531d7a5` (7/24, accessibility + policies). Re-verify gate caught this before any duplicate work was done.
- **`rancho-seo-s4-schema-breadcrumb-speakable` → RESOLVED.** Full sitewide re-audit across all 29 public pages: **0 invalid JSON-LD, 0 pages without `speakable`.** Sitemap ↔ routes ↔ page-files coverage verified complete in all three directions (29 routes / 30 sitemap entries incl. apex / 29 page files; nothing routed-but-unlisted, nothing listed-but-unrouted, no page file without a route). The schema runway is genuinely exhausted this time — the 5/26 "exhausted" call was cluster-scoped and wrong; this one is whole-site and measured.
- **Shipped — internal-linking floor restoration (`rancho-seo-s3`, PROGRESS held 95%).** `ranch-wedding-texas.html` was the single thinnest post in the 17-post cluster at **3 inbound topical links**, against the floor of 4 held by every other post since the 5/10 close-out. Checked against `HEAD` first: the inbound set is byte-identical in the working tree and at `HEAD`, so this is a **long-standing gap, not a regression** — reported as such. Added one `<li>` to the Related Reading list in `bachelorette-party-austin-texas.html` → `/blog/ranch-wedding-texas/`. Host picked for link equity (8 inbound) and because it already carried a `wedding-venues-near-austin` link, so the bachelorette→wedding adjacency was editorially established; placed adjacent to it. Anchor text uses the target's exact title, while the **three existing inbound links use three different phrasings — that variance was deliberately preserved**, since uniform exact-match anchors across every inbound link is an over-optimization footprint. **Cluster floor now uniform 4 across all 17 posts.** 2 files, +1 line; `dateModified` 2026-05-26 → 2026-08-16, sitemap `lastmod` synced for that one URL with replacement count asserted `== 1`.
- **Verified:** `npm run validate:site` passes; 4/4 JSON-LD blocks on the edited page re-parse; cluster floor recomputed post-edit = 4; sitemap re-parsed well-formed.
- **Two false alarms caught and killed before reporting, both self-inflicted:** `pool-day-pass-austin.html` appeared to carry invalid JSON-LD (actually my audit script mishandled `"@type": ["WebPage","ReservePage"]`, a list not a string); and `/pool-day-pass-austin/` appeared to 404 (the canonical path is `/blog/pool-day-pass-austin/`, which is 200 and correctly in the sitemap).
- **FLAG_FOR_ADAM — dead YouTube video live on `/videos/`, day 24.** Card "Escape to Rancho Moonrise" (`_klefu2vTwM`) re-verified today: thumbnail **404**, oembed **403**; the other three IDs on the page return 200/200. Renders a broken thumbnail to real visitors. First flagged 7/23, still unactioned — now the oldest live user-facing defect on the site. Card left in place rather than silently deleting visible content.
- **Hygiene:** untracked duplicate files are accumulating (`weddings 2.html`, `CONTEXT 2/3.md`, `TODO 2/3/4.md`, `CHANGELOG 2/3/4.md`, `styles 2.css`, several `run-logs/... 2.md`). All untracked, so none deploy — flagged for a deliberate sweep, not touched. Pre-existing uncommitted prior-session changes intentionally NOT staged; committed by explicit path. Run-log: `run-logs/2026-08-16-seo.md`.

## 2026-07-23 (rancho-review-monitor) — RUN_067: 34th quiet sweep on counts; 2 material follow-ups

- **Expedia/Hotels.com two-entity split RE-CONFIRMED — 2nd consecutive run.** RUN_066 upgraded the recurring 9.0 from "transient artifact" to "probable real divergence." This run re-confirms it across independent queries, per-entity again: `expedia.com/…h89565924` → **8.0** ("a solid guest review rating of 8.0"); `hotels.com/ho2867109568` (and `ie.hotels.com`) → **9.0 "Wonderful."** Two consecutive runs of consistent per-entity attribution makes the artifact reading (held for 10 prior runs) no longer tenable. The **8.0 anchor was again deliberately NOT overwritten** — the hard rule forbids editing `brand/review-aggregate.json` without a fresh live scrape, and hotels.com direct fetch remains a 42-timeout BLOCKER. `entity_split_hypothesis` status bumped to RE-CONFIRMED. **FLAG_FOR_ADAM strengthens:** 30 seconds in either extranet settles whether this is one rating or two — and if two, `hotels.com/ho2867109568` may be accumulating reviews on a surface nobody is monitoring for replies. The RUN_066 first-ever count signals (13, 20) did **not** resurface this run — count stays null in `count_signals_unconfirmed`.
- **Done-log path corrected.** Prior runs recorded the unreplied signal as "`brand/rancho-done-log.md` does not exist." The operative done-log is at repo **root** (`rancho-done-log.md`, per the Mark Done System + Key Files) and it **does** exist. Grepped directly this run: the last review-reply resolution is `2026-04-15 [rancho] rancho-review-replies — RESOLVED` (9 Google replies) — **no Cassie Butterfield and no Haylee L. entry**. Substance is unchanged (both drafts genuinely unposted), but future runs should check the root file, and any RESOLVED line the monitor writes lands there.
- **Re-Verify Gate: 11 live claims — 11 still_true** (Expedia split divergent-but-stable), 0 partial, 0 new-signal, 0 failed, **0 fully resolved.** No RESOLVED line written — the root done-log carries no Cassie/Haylee resolution, so both unreplied counts hold.
- **No new reviews on any monitored platform.** Google snippet **oscillated 126 → 175** (back to the high value after 2 runs at 126; 4.9★ not inline this query) — the live-authoritative 130 is now **65 days stale**, and the snippet has printed 126, 130 and 175, so it is not a proxy in either direction. Facebook 5/100% (48th consecutive inline). The Knot 8/4.5★ surfaced inline; Haylee L.'s 1★ still live ("would never recommend" + loud-music body) with **no owner-response** — the only "Haylee" owner-reply on The Knot is for a **different** venue (The Old Rancho, Carlsbad CA) — **147 days / ~21 weeks unreplied**. TripAdvisor 0/unclaimed held via search corroboration. Hipcamp count held 0 (no contradicting review-content signal this run; no-attempt cycle resumes at 1); voice violations "34-acre ranch" (VOICE-GUIDE says 36) and "bar" re-confirmed live.
- **Two drafted replies remain unposted at day 66** (Cassie Butterfield Google 5★; Haylee L. The Knot 1★). No auto-publish — drafts stay at `brand/review-reports/2026-05-19-review-report.md` for Adam/Ashley.
- **Hygiene flag:** CONTEXT.md is at 180 lines (over the 150 cap) — driven by stacked site-daily/competitive-weekly Last-Worked-On bullets, not review-monitor content (this run's edit was net-neutral). Left to those tasks' owners to avoid clobbering concurrent writers.
- **No new blocker opened.** No-attempt counters: airbnb (66), hotels.com (42, held), theknot-direct (33), tripadvisor-direct (2nd), hipcamp-direct (cycle resumes at 1). State: aggregate 66→67, dashboard-state 66→67. Run-log: `run-logs/2026-07-23-review-monitor.md`.

## 2026-07-22 (rancho-review-monitor) — RUN_066: 33rd quiet sweep on counts, but 2 material data events

- **Expedia/Hotels.com "9.0 artifact" reclassified as a probable real two-entity split.** For 10 consecutive runs the monitor treated a recurring 9.0 value as transient snippet noise against an 8.0 anchor. This run 9.0 surfaced **three times across independent queries** and — for the first time — was **consistently attributed per entity**: `hotels.com/ho2867109568` → **9.0 "Wonderful"**, `expedia.com/Manor-Hotels-Rancho-Moonrise.h89565924` → **8.0**. That is no longer noise-shaped. The **8.0 anchor was deliberately NOT overwritten** — the hard rule forbids editing `brand/review-aggregate.json` without a fresh live scrape, and hotels.com direct fetch remains a 42-timeout BLOCKER. Recorded instead as an `entity_split_hypothesis` block pending confirmation, and flagged to Adam as a 30-second extranet check. **If the two entities genuinely differ, reviews may be accumulating on a surface nobody is monitoring for replies.**
- **First review counts for the Expedia/Hotels.com cluster since the null baseline (2026-04-09)** — **13** (hotels.com query) and **"8.6 out of 10 from 20 reviews"** (Manor RV-parks travel-guide query). Mutually inconsistent; the 8.6/20 came off an *aggregate* travel-guide page and carries the same cross-listing smell as the RUN_061 "Travelers' Choice" artifact. Logged to `count_signals_unconfirmed`, **not** written as the count.
- **Hipcamp 22-run no-attempt cycle deliberately broken on a contradicting signal.** A WebSearch snippet surfaced apparent **review content** against the claimed `0 reviews` baseline. The Re-Verify Gate forbids assuming a claim still holds when contradicted, so the blocker's no-attempt cycle was broken and a direct fetch attempted — it **failed again** (`"Loading…"`, identical to the RUN_043 failure mode); failure count 3→4, cycle resets. **Count held at 0**: the quotes are not attributable to Hipcamp — the same query returned an unrelated **`Stargazing @ Moonrise Ranch` (Mountain Center, California)** listing, and the mosquito quote resurfaced in a Hotels.com-dominated result set. Reading: the summarizer blended Hipcamp *listing copy* with Expedia/Hotels.com *review content*. Not written to the aggregate.
- **Two long-stale Hipcamp voice violations re-confirmed live after 57 days.** `"34-acre ranch"` (VOICE-GUIDE says 36) and `"a bar"` both surfaced in today's snippets. They had carried `STALE:2026-05-26` since the last successful scrape and are now current — a staleness resolution, not a new finding.
- **Re-Verify Gate: 11 live claims** — 8 still_true, 1 partial/divergent (Expedia rating), 1 new-signal (Expedia counts), 1 failed (Hipcamp count), **0 fully resolved**. No RESOLVED line written: `brand/rancho-done-log.md` still does not exist, so both unreplied counts hold on the done-log-absence signal.
- **No new reviews on any monitored platform.** Google snippet held at 126 (2 runs stable, 4.9★ inline) — but the live-authoritative 130 is now **64 days stale**, and the snippet has printed 126, 130 and 175, so it is not a proxy in either direction. Facebook 5/100% (47th consecutive inline). The Knot 8/4.5★ carried; Haylee L.'s 1★ body still indexed **verbatim on a name-free query** with no owner-response anywhere — **146 days / ~21 weeks unreplied**. TripAdvisor 0/unclaimed held via search corroboration.
- **Two drafted replies remain unposted at day 65** (Cassie Butterfield Google 5★; Haylee L. The Knot 1★). No auto-publish — drafts stay at `brand/review-reports/2026-05-19-review-report.md` for Adam/Ashley.
- **CONTEXT.md hygiene — this task collapsed its own history**, as flagged by the 2026-07-22 site-daily run: 10 stacked `review-monitor` "Nth consecutive quiet sweep" entries replaced by one RUN_034–065 summary line. CONTEXT.md **205 → 181 lines**. No detail lost — per-run records are append-only in `tasks/review-monitor/session-log.md`.
- **No new blocker opened.** State: aggregate 65→66, dashboard-state 65→66. Run-log: `run-logs/2026-07-22-review-monitor.md`.

## 2026-07-22 (rancho-content-weekly) — 10-run block BROKEN; first strengthening pass shipped on `accommodations.html`

- **Both structural hard stops re-checked live; one cleared, one still open.** GOALS.md (re-read from disk, not inherited from CONTEXT.md) now carries a `## Settled Decisions` section: **"Rancho Moonrise author byline: Ashley … Byline form: `Ashley · Rancho Moonrise`. Settled 2026-07-15."** That closes the author-block hard stop that blocked runs 1–9 (4/22 → 7/15). `brand/approved-testimonials.md` now exists (created 2026-07-15) but its sole candidate **T-001 (Cassie Butterfield, Google 5★) is `STATUS: UNAPPROVED`**, and the file's own rule permits pulling only `APPROVED` entries — so the testimonial element stays blocked. With photos + author + local detail available, the spec's **"at least 3 of 4"** bar is met for the first time in 10 weeks and the pass shipped.
- **Shipped — `accommodations.html` strengthened** (1 file). Three of four required elements landed:
  - **3 real property photos, all verified by opening the image files** (no stock, no AI, no alt text written from a filename). (1) Pet Friendly card `accom-safari-tent` → `lodge-window-view` — a black ranch dog at the tall Lodge window watching the pasture; (2) WiFi card `accom-group-aerial` → `lodge-dining-area` — the long reclaimed-wood table and lounge; (3) new bathhouse photo `accom-outdoor-bath` placed in a real content section. Cards 1 and 2 were **duplicates of images already shown elsewhere on the same page** — the page rendered `accom-safari-tent`, `accom-group-aerial` and `accom-cabin-exterior` twice each.
  - **Named author block** — `Ashley · Rancho Moonrise` + a one-line role description and review date, per the GOALS.md settled form. This is the **first visible author block anywhere on the site**; no prior page renders one and no CSS class existed, so it uses the page's existing inline-style convention. Matching `author` (Person → `worksFor` Organization) added to the `WebPage` JSON-LD, alongside `dateModified: 2026-07-22`.
  - **Local detail a competitor can't copy** — replaced the generic **"clean, well-maintained bathhouse"** (which appeared 3× on this page and is copyable by any venue in Texas) with the real structure: corrugated metal walls, cedar plank floor, a carved river-rock sink on a cedar vanity, walk-in shower. Verified by opening `accom-outdoor-bath-1024.webp` directly, not inferred from alt text. Added the Travis County active-burn-ban constraint on fire pits (sourced from `faqs.html`, already published) and the donkeys at the fence line (VOICE-GUIDE-sanctioned phrasing).
- **Bug fixed in passing — the social/AI share card for this page was a thumbnail.** `og:image` and `twitter:image` pointed at `accommodation-premium-safari.jpg`, which is **342×340**, while declaring `og:image:width 1200` / `og:image:height 630`. Every share, preview and AI-engine card fetch got a near-thumbnail with lying dimensions. Repointed to `accom-safari-tent.jpg` (**2049×1536**, 1.1 MB) with true declared dimensions. Checked the other 29 public pages — **no other page has this defect**; it was isolated here. (`accom-cabin-exterior.jpg` is the higher-res option at 6000×4000 but is 16 MB, over common scraper limits.)
- **Verification:** `npm run validate:site` passes; all 3 JSON-LD blocks parse clean with `dateModified` + `author.name = Ashley` asserted; all 4 referenced assets return 200 and decode at real dimensions (`accom-safari-tent.jpg` measured 2049×1536, matching the declared OG values exactly); zero banned words (`luxury` / `Hill Country` / `General Store` / `picture-perfect` / `nestled` / `unforgettable` / `rolling hills` / `breathtaking` / `hidden gem`); zero emoji; `Manor` appears only in the schema `addressLocality` and the footer postal address, never as a location descriptor. DOM-level check confirms the new section renders with correct heading, alt text, author text and position (immediately before the CTA banner).
- **Could not produce a screenshot** — the preview harness reported `window.innerHeight: 0`, so the viewport never had height, IntersectionObserver could not fire and `loading="lazy"` images never decoded in-page. Confirmed this is a harness artifact and **not a regression**: every pre-existing `.fade-in` image on the page behaves identically, and the untouched homepage reveals normally. Verified the assets independently via `new Image()` decode instead.
- **Galleries are Supabase-hydrated — noted for future runs.** Both `[data-gallery]` blocks on this page (`ranch_tour`, 12 active rows; `lodge`, 8) call `el.innerHTML = ''` before appending rows from `rancho_photos`. **Any static tile added to those galleries is wiped client-side in production.** New photos must go in non-hydrated slots; this run placed all three accordingly.
- **Not done, logged instead:** testimonial element (T-001 needs Adam's approval + an event date from Ashley; nothing on file serves accommodations at all). Two orphaned real assets found — `corral-hank-willie` / `corral-waylon-texas`, in the responsive ladder with **zero references anywhere in the repo** — left unplaced because VOICE-GUIDE lists the horse corral under "future, not yet public-ready" while it is already live on 3 production pages; that contradiction is Adam's to settle, not an autonomous run's.

## 2026-07-22 (rancho-site-daily) — cruise-control streak BROKEN at 39; FAQPage + Speakable shipped on the last uncovered blog page

- **Pause gate corrected — the 39-run cruise-control read was stale.** GOALS.md (mtime 2026-07-20 11:19) no longer says "No Rancho Moonrise active work — paused." It now reads "No Rancho Moonrise **outreach** (Christopher Gill) — still paused. **Site/content work RESUMED 2026-07-15**," and the Paused-Workstreams entry says the same. The pause was narrowed to outreach only; the 7/17 and 7/18 runs carried the old text forward and idled through an unblocked workstream. Autonomous site/schema work is in scope again as of this run.
- **Shipped — `/blog/weekend-getaways-near-austin/` brought to blog-cluster schema parity** (2 files, +58/-2, schema-only, zero copy changes). A per-page JSON-LD coverage matrix across all 30 public pages found this page was the sole cluster member carrying `BlogPosting` + `BreadcrumbList` but **no** `WebPage`/`speakable` and **no** `FAQPage` — despite already rendering a complete visible `.aeo-block` Quick-Answers section with 4 Q&A pairs. Real content, zero machine-readable exposure. Added: `WebPage` + `SpeakableSpecification` (selector array byte-identical to the other 16 pages) and `FAQPage` with 4 Q&As **extracted programmatically from the rendered DOM, not authored** — Google treats FAQ markup that doesn't match visible text as a policy violation, so the script parses the page and asserts round-trip equality. `BlogPosting.dateModified` 2026-05-26 → 2026-07-22; sitemap `lastmod` synced for that one URL only.
- **Verification:** `npm run validate:site` passes; 5/5 JSON-LD blocks parse clean on the target; 4/4 FAQ Q&A assert verbatim against visible on-page text; 0 invalid JSON-LD blocks across all 30 public pages. Blog cluster **16/17 → 17/17** for both Speakable and FAQPage.
- **Re-Verify Gate (live, 7/7 still_true, 0 resolved):** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 78168` ≈ 21.7 hr, `last-modified` Tue 21 Jul 17:16 GMT); www 308 → apex; sitemap / corp-retreats / safari-tents / blog / robots all 200.
- **AggregateRating-on-BlogPosting candidate RETIRED, not deferred.** Per `memory/reference_aggregate_rating_self_serving.md`, self-serving review schema earns no SERP stars — it had sat as the pre-scoped backup slot since 5/26 and should not be re-proposed.
- **NEEDS ADAM #0 (scheduled-tasks-vs-pause-list, open since 5/27) resolved by the GOALS.md update** — site/content work is unpaused, so `rancho-site-daily` firing daily is now correct behavior, not drift. Logged to done-log.
- **Coverage gap noted:** no `rancho-site-daily` run-log exists for 7/19, 7/20 or 7/21 — the task did not fire (or fired without logging) for three days. Not reconstructable. Remaining schema gaps logged to TODO: `videos.html` carries no JSON-LD at all; `accessibility.html` / `policies.html` lack Speakable. Run-log: `run-logs/2026-07-22-seo.md`.
...
- **Re-Verify Gate (8 live claims) — 7 still_true, 1 STALE, 0 resolved:** TripAdvisor DIRECT WebFetch of canonical `g56224-d33307272` returned **HTTP 403 for the 2nd consecutive run** (RUN_063 1st) → watch escalated to **2-of-3**; BLOCKER opens at 3 (if 403 recurs RUN_065). WebSearch corroborates canonical listing indexed → 0/unclaimed HELD; price band NOT re-confirmable, carries `$45–$154` STALE:2026-06-30 (algorithmic rate, not a review signal). Google snippet OSCILLATED 126 → 175 (high value; non-authoritative; not a reliable proxy either direction; live-authoritative 130 STALE 58d). Expedia 8.0 SURFACED INLINE (recovered after RUN_063 miss; no 9.0 artifact, 9th run without). The Knot 8/4.5★ surfaced inline (count held — no new reviews); Haylee L. 1★ body still indexed name-free, no owner-response (**140 days unreplied**, 20wk). Facebook 5/100% (45th inline). BLOCKERS held no-attempt: theknot-direct-fetch (30th), hipcamp-direct-fetch (21st cycle run), airbnb 403 (63rd), hotels.com (counter 42), google-count.
- **Carry-forward (unposted):** 2 RUN_034 drafts remain — Cassie Butterfield Google 5★ (day 59 in monitor) + Haylee L. Knot 1★ (140d unreplied, day 59 in monitor); both at `brand/review-reports/2026-05-19-review-report.md`. Mark `rancho-review-replies-2026-05-20` done from briefing after posting. Dashboard status held `pending`.
- **State:** aggregate 63→64 (RUN_063 folded into `note_prior_run_063`), dashboard-state 63→64, `run-logs/2026-07-16-review-monitor.md` written. Clean continuity — RUN_063 committed cleanly 7/04, no orphan; today's site-daily (37th firing) committed CONTEXT/CHANGELOG/TODO at `958b3fe` first (clean, edited fresh here). Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged.

## 2026-07-16 (rancho-site-daily) — 37th consecutive cruise-control firing

- **No shipped edits.** GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"), last touched 2026-07-02 (LoanOS-only; Rancho line unchanged). No Ashley move this cycle. Surgical CreativeWork-property enrichment runway on BlogPosting JSON-LD closed 5/26 — no autonomous slot remains; Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- **Re-Verify Gate (live) — 7/7 still_true, 0 resolved:** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 66592` ≈ 18.5 hr); www 308 → apex; sitemap.xml / `/corporate-retreats/` / `/safari-tents-near-austin/` / `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 51 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026). 0 stale claims → no done-log entry.
- **Clean continuity:** `run-logs/2026-07-15-seo.md` (yesterday's 36th firing) tracked — no orphaned prior-day SEO log; CHANGELOG.md working tree == HEAD (no concurrent-writer stale-revert). Ownership boundaries unchanged: indexing/SERP → competitive-weekly + Adam; review/GBP → review-monitor/gbp-weekly; bi-weekly audit (last 6/18, due 7/2) → audit task. Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged. Run-log: `run-logs/2026-07-16-seo.md`.

## 2026-07-15 (rancho-site-daily) — 36th consecutive cruise-control firing

- **No shipped edits.** GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"), last touched 2026-07-02 (LoanOS-only; Rancho line unchanged). No Ashley move this cycle. Surgical CreativeWork-property enrichment runway on BlogPosting JSON-LD closed 5/26 — no autonomous slot remains; Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- **Re-Verify Gate (live) — 7/7 still_true, 0 resolved:** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 877067` ≈ 10.15 days — no redeploy since ~7/5, consistent with no shipped edits); www 308 → apex; sitemap.xml / `/corporate-retreats/` / `/safari-tents-near-austin/` / `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 50 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026). 0 stale claims → no done-log entry.
- **Continuity fix:** the 7/6 run (35th firing) wrote `run-logs/2026-07-06-seo.md` but its four-file edits never committed — the concurrent notebooklm-sync task committed over the top at `919653a`. This run commits that orphaned 7/6 log by explicit path alongside today's. Last committed site-daily run was 7/5 (34th) at `1803e02`; gap 7/7–7/14 (task did not fire/commit). Ownership boundaries unchanged: indexing/SERP → competitive-weekly + Adam; review/GBP → review-monitor/gbp-weekly; bi-weekly audit (last 6/18, due 7/2) → audit task. Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged. Run-log: `run-logs/2026-07-15-seo.md`.

## 2026-07-05 (rancho-site-daily) — 34th consecutive cruise-control firing

- **No shipped edits.** GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"), last touched 2026-07-02 (LoanOS-only; Rancho line unchanged). No Ashley move this cycle. Surgical CreativeWork-property enrichment runway on BlogPosting JSON-LD closed 5/26 — no autonomous slot remains; Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- **Re-Verify Gate (live) — 7/7 still_true, 0 resolved:** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 45882` ≈ 12.7 hr); www 308 → apex; sitemap.xml / `/corporate-retreats/` / `/safari-tents-near-austin/` / `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 40 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026). 0 stale claims → no done-log entry.
- **Continuity:** 7/4 SEO log committed in `cb3046a`; no orphan to reconcile; CONTEXT/CHANGELOG/TODO clean at HEAD == working tree at start. Ownership boundaries unchanged: indexing/SERP → competitive-weekly + Adam; review/GBP → review-monitor/gbp-weekly; bi-weekly audit (last 6/18, due 7/2) → audit task. Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged. Run-log: `run-logs/2026-07-05-seo.md`.

## 2026-07-04 — Review Monitor RUN_063 (30th quiet sweep; TripAdvisor direct-fetch 403 1st failure; Google snippet held 126; Expedia 8.0 not inline; Facebook 44th)

- **rancho-review-monitor RUN_063:** Thirtieth consecutive quiet platform sweep — no new reviews on any monitored platform, no rating/count drops, no new ≤3★. Cruise-control maintenance run under GOALS.md week-of-5/18 broad Rancho pause (review-monitor not in the Scheduled-Tasks Pause List; monitoring ≠ active work). Two RUN_034 drafts still unposted day 47 in monitor (Cassie Butterfield Google 5★; Haylee L. The Knot 1★ 128 days unreplied); done-log shows no review-related entry since 2026-04-15 22:12. **Re-Verify Gate: 7/8 still_true, 1 STALE, 0 resolved** (no done-log RESOLVED line). **NEW — TripAdvisor DIRECT WebFetch of canonical `g56224-d33307272` returned HTTP 403** — 1st failure of the previously-reliable direct path (RUN_060/061/062 all succeeded); BLOCKER threshold is 3, none opened; logged as a `watch` item in dashboard-state. Fell back to WebSearch — canonical listing still indexed ("RANCHO MOONRISE - Prices & Campground Reviews") with no count/rating in snippet → **0/unclaimed HELD**; price band **NOT re-confirmable**, carries `$45–$154` **STALE:2026-06-30** (algorithmic standard-room rate estimate, not a review signal). **Google** WebSearch snippet **held at 126**/4.9★ (2-run hold; non-authoritative; live 130/4.9★ carries from RUN_034 Chrome read, now 46 days STALE). **Expedia** 8.0 anchor **did NOT surface inline** this run (targeted Hotels.com/Expedia query returned no numeric rating) — 8.0 carried, absence NOT asserted as a change per the Re-Verify Gate (reliability downgraded since RUN_050; breaks the 32-run inline-number streak); Hotels.com listing `ho2867109568` still active. **Facebook** 5/100% INLINE 44th consecutive run. **The Knot** Haylee L. 1★ body still indexed in Rancho-attributed snippet via name-free query (name-surfacing not asserted, no owner-response indexed, 128 days unreplied); 8/4.5★ carried (not inline); direct fetch not attempted (BLOCKER `theknot-direct-fetch`, 29th no-attempt run). **Hipcamp** direct fetch not attempted (BLOCKER `hipcamp-direct-fetch`, 20th no-attempt-cycle run); voice/data violations carry STALE:2026-05-26. **Airbnb** 403 BLOCKER 62nd no-attempt; **Hotels.com** timeout BLOCKER (counter holds 42, cruise-control). Dashboard status: `pending`. No new review-report produced (no new reviews to draft); both carry-forward drafts remain at `brand/review-reports/2026-05-19-review-report.md`. State updated: `brand/review-aggregate.json` (62→63) + `site/admin/dashboard-state.json` (62→63) + `run-logs/2026-07-04-review-monitor.md`. Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged.

## 2026-07-04 (rancho-site-daily) — 33rd consecutive cruise-control firing

- 33rd consecutive `rancho-site-daily` cruise-control firing. No shipped edits. GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"), last touched 2026-07-02 (LoanOS-only update; Rancho line unchanged); no Ashley move this cycle.
...
- Re-Verify Gate (live, 7/7 still_true, 0 resolved): apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 32940` ≈ 9.15 hr); www 308 → apex; sitemap / corp-retreats / safari-tents / blog all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 39 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- Clean continuity: 7/3 SEO log committed in `eb6df83`; no 7/4 orphan to reconcile; CONTEXT.md/CHANGELOG.md/TODO.md clean at HEAD == working tree at start. Note: bi-weekly business audit due 7/2 per CONTEXT — owned by the audit task, not site-daily; not run here.
...
- Run-log: `run-logs/2026-07-04-seo.md`. No SEO task IDs (s1–s8) touched, no done-log entry.

## 2026-07-03 (rancho-site-daily) — 32nd consecutive cruise-control firing

- 32nd consecutive `rancho-site-daily` cruise-control firing. No shipped edits. GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"), last touched 2026-07-02 (LoanOS-only update; Rancho line unchanged); no Ashley move this cycle.
- CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26 — no surgical autonomous slot remains. Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- Re-Verify Gate (live, 7/7 still_true, 0 resolved): apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 11239` ≈ 3.1 hr); www 308 → apex; sitemap / corp-retreats / safari-tents / blog all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 38 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- Clean continuity: 7/2 SEO log committed in `bafd732`; no 7/3 orphan to reconcile; CONTEXT.md HEAD == working tree at start. Note: bi-weekly business audit was due 7/2 per CONTEXT — owned by the audit task, not site-daily; not run here.
- Pre-existing prior-session changes (api/inquiry.js, styles.css, main.js, contact.html, weddings.html) + concurrent-writer artifacts (`*" 2".*`/`*" 3".*`, `TODO.md.tmp.*`, `AGENTS.md`, `CHANGELOG 2.md`, `TODO 2.md`, `rancho-moonrise-assets/`, `youtube-uploads/`, duplicated images) intentionally NOT staged. Commit by explicit path only.
- Run-log: `run-logs/2026-07-03-seo.md`. No SEO task IDs (s1–s8) touched, no done-log entry.
...
- Re-Verify Gate (live, 7/7 still_true, 0 resolved → no done-log RESOLVED line): apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 25044` ≈ 7.0 hr); www `308` → `https://ranchomoonrise.com/`; sitemap.xml 200; `/corporate-retreats/` 200; `/safari-tents-near-austin/` 200; `/blog/` 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 31 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
...
- Clean continuity: CHANGELOG.md HEAD == working tree (no concurrent-writer stale-revert this cycle); `run-logs/2026-06-25-seo.md` tracked, no orphaned prior-day SEO log. Pre-existing prior-session changes (api/inquiry.js, styles.css, main.js, contact.html, weddings.html) + concurrent-writer artifacts (`* 2`/`* 3`, AGENTS*.md, TODO.md.tmp.*, duplicated images) intentionally NOT staged; committed by explicit path only.
...
- Cruise-control maintenance run under GOALS.md week-of-5/18 broad Rancho pause ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"). No Ashley move this cycle. No shipped edits.
- Re-Verify Gate (live, 7/7 still_true, 0 resolved → no done-log RESOLVED line): apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 32645` ≈ 9.1 hr); www `308` → `https://ranchomoonrise.com/`; sitemap.xml 200; `/corporate-retreats/` 200; `/safari-tents-near-austin/` 200; `/blog/` 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 29 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26 — no surgical autonomous slot remains. Person-author byline research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- Ownership boundaries: indexing claims (`/corporate-retreats/` ~46d, `/safari-tents-near-austin/` ~51d uncrawled) owned by competitive-weekly + Adam — not re-litigated; review/GBP claims owned by review-monitor/gbp-weekly — not touched.
- Continuity: CHANGELOG.md HEAD == working tree (no concurrent-writer stale-revert); `run-logs/2026-06-23-seo.md` tracked, no orphaned prior-day SEO log. Pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged; committed by explicit path only. Run-log: `run-logs/2026-06-24-seo.md`.

## 2026-06-23 (rancho-site-daily) — 24th consecutive cruise-control firing, no shipped edits

- Cruise-control maintenance run under GOALS.md week-of-5/18 broad Rancho pause ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"). No Ashley move this cycle. No shipped edits.
- Re-Verify Gate (live, 7/7 still_true, 0 resolved → no done-log RESOLVED line): apex `HTTP/2 200` + `server: Vercel` + `x-vercel-cache: HIT` (`age: 7246` ≈ 2.0 hr); www `308` → `https://ranchomoonrise.com/`; sitemap.xml 200; `/corporate-retreats/` 200; `/safari-tents-near-austin/` 200; `/blog/` 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 28 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26 — no surgical autonomous slot remains. Person-author byline research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- Ownership boundaries: indexing claims (`/corporate-retreats/` ~46d, `/safari-tents-near-austin/` ~51d uncrawled) owned by competitive-weekly + Adam — not re-litigated; review/GBP claims owned by review-monitor/gbp-weekly — not touched.
- Clean continuity: CHANGELOG.md HEAD == working tree (no concurrent-writer stale-revert this cycle); `run-logs/2026-06-22-seo.md` tracked, no orphaned prior-day SEO log. Pre-existing prior-session changes (api/inquiry.js, styles.css, main.js, contact.html, weddings.html) + concurrent-writer artifacts (`* 2.*`, `* 3.*`, AGENTS.md, TODO.md.tmp.*) intentionally NOT staged; committed by explicit path only.
- No SEO task IDs (s1–s8) touched, no done-log entry. Run-log: `run-logs/2026-06-23-seo.md`.
...
- GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"). No Ashley move this cycle. CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26 — no surgical autonomous slot remains; Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- Re-Verify Gate (live, 7/7 still_true, 0 resolved): apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 60969` ≈ 16.9 hr); www 308 → apex; `/sitemap.xml`, `/corporate-retreats/`, `/safari-tents-near-austin/`, `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 26 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- Continuity fix: committed orphaned `run-logs/2026-06-20-seo.md` (written by the 6/20 run, never committed — concurrent writer committed over the top) by explicit path.
- Ownership: indexing claims owned by `rancho-competitive-weekly` + Adam; review-count / GBP-reply / GBP-cadence claims owned by `rancho-review-monitor` / `rancho-gbp-weekly` — neither re-litigated here.
- No site/pages/* edits, no schema edits, no sitemap touch. Pre-existing prior-session changes (api/inquiry.js, styles.css, main.js, contact.html, weddings.html) + concurrent-writer artifacts intentionally NOT staged. Committed by explicit path only. No SEO task IDs (s1–s8) touched, no done-log entry. Run-log: `run-logs/2026-06-21-seo.md`.

## 2026-06-20 (rancho-site-daily) — 21st consecutive cruise-control firing, no shipped edits

- GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"). No Ashley move this cycle. CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26 — no surgical autonomous slot remains; Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- Re-Verify Gate (live, 7/7 still_true, 0 resolved): apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 16250` ≈ 4.5 hr); www 308 → apex; `/sitemap.xml`, `/corporate-retreats/`, `/safari-tents-near-austin/`, `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 25 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- Ownership: indexing claims owned by `rancho-competitive-weekly` + Adam; review-count / GBP-reply / GBP-cadence claims owned by `rancho-review-monitor` / `rancho-gbp-weekly` — neither re-litigated here.
- No site/pages/* edits, no schema edits, no sitemap touch. Pre-existing prior-session changes (api/inquiry.js, styles.css, main.js, contact.html, weddings.html) + concurrent-writer artifacts intentionally NOT staged. Committed by explicit path only. No SEO task IDs (s1–s8) touched, no done-log entry. Run-log: `run-logs/2026-06-20-seo.md`.

## 2026-06-19 (rancho-site-daily) — 20th consecutive cruise-control firing, no shipped edits

- GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused; cruise control only if Ashley moves"). No Ashley move this cycle. CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26 — no surgical autonomous slot remains; Person-author research + AggregateRating-on-BlogPosting both deferred again (each adds NEEDS-ADAM noise or a 17-file diff against a paused workstream).
- Re-Verify Gate (live, 7/7 still_true, 0 resolved): apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 58194` ≈ 16.2 hr); www 308 → apex; `/sitemap.xml`, `/corporate-retreats/`, `/safari-tents-near-austin/`, `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 24 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- Ownership: indexing claims owned by `rancho-competitive-weekly` + Adam; review-count / GBP-reply / GBP-cadence claims owned by `rancho-review-monitor` / `rancho-gbp-weekly` — neither re-litigated here.
- No site/pages/* edits, no schema edits, no sitemap touch. Pre-existing prior-session changes (api/inquiry.js, styles.css, main.js, contact.html, weddings.html) + concurrent-writer artifacts intentionally NOT staged. Committed by explicit path only. No SEO task IDs (s1–s8) touched, no done-log entry. Run-log: `run-logs/2026-06-19-seo.md`.
...
- CreativeWork-property enrichment arc on BlogPosting JSON-LD closed 5/26; no surgical autonomous slot remains. Person-author research (new NEEDS ADAM byline matrix) + AggregateRating-on-BlogPosting (17-file cluster diff) both deferred again — shipping either adds review-noise against a paused workstream, not signal.
- **Re-Verify Gate (live, 7/7 still_true, 0 resolved):** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 41782` ≈ 11.6 hr); www 308 → apex; sitemap / `/corporate-retreats/` / `/safari-tents-near-austin/` / `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 21 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026, copyrightHolder "Rancho Moonrise").
- Indexing claims owned by `rancho-competitive-weekly` + Adam — not re-litigated. No SEO task IDs (s1–s8) touched; no done-log entry.
- NEEDS ADAM #0 pause-list fix now **20 days overdue** (first surfaced 5/27); `rancho-site-daily` alone has fired in cruise-control mode 17 consecutive times. Strengthened in TODO.md.
- Pre-existing prior-session changes (api/inquiry.js, styles.css, main.js, contact.html, weddings.html) + concurrent-writer artifacts (`* 2`/`* 3` dupes, TODO.md.tmp.*, AGENTS*.md, rancho-moonrise-assets/, duplicated images) intentionally NOT staged. Committed by explicit path only — no `git add -A`. Run-log: `run-logs/2026-06-16-seo.md`.
...
- **Re-Verify Gate (live, 7/7 still_true, 0 resolved):** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 2064` ≈ 34 min); www 308 → apex; sitemap / corp-retreats / safari-tents / blog all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 19 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026). 0 stale claims → 0 done-log entries.
- GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused"; 6/6 freeze-lift was styermortgage.com only). Indexing claims (`/corporate-retreats/`, `/safari-tents-near-austin/` uncrawled) owned by competitive-weekly — not re-litigated. Review/GBP claims owned by review-monitor / gbp-weekly — not touched.
- Pre-existing prior-session changes (api/inquiry.js, site/css/styles.css, site/js/main.js, site/pages/contact.html, site/pages/weddings.html) + concurrent-writer artifacts (`* 2`/`* 3` dups, AGENTS.md, TODO.md.tmp.*, rancho-moonrise-assets/, duplicated images, youtube-uploads/) intentionally NOT staged. Committed by explicit path only — no `git add -A`. No site/pages/* edits, no schema touch, no sitemap touch. No SEO task IDs (s1–s8) touched. Run-log: `run-logs/2026-06-14-seo.md`.

## 2026-06-13 (rancho-site-daily) — 14th consecutive cruise-control firing, no shipped edits

- 14th consecutive `rancho-site-daily` cruise-control firing. No shipped edits. CreativeWork-property enrichment arc on the 17-post BlogPosting JSON-LD cluster closed 5/26 — no surgical autonomous slot remains. Both pre-scoped candidates (Person-author research, AggregateRating-on-BlogPosting) deferred again: one surfaces a new NEEDS ADAM, the other is a 17-file cluster diff against a workstream GOALS.md still pauses.
- **Re-Verify Gate (live, 7/7 still_true, 0 resolved):** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 50852` ≈ 14.1 hr); www 308 → apex; sitemap / corp-retreats / safari-tents / blog all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 18 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026). 0 stale claims → 0 done-log entries.
- GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused"; 6/6 freeze-lift was styermortgage.com only). Indexing claims (`/corporate-retreats/`, `/safari-tents-near-austin/` uncrawled) owned by 6/8 competitive-weekly — not re-litigated. Review/GBP claims owned by review-monitor / gbp-weekly — not touched.
- Pre-existing prior-session changes (api/inquiry.js, site/css/styles.css, site/js/main.js, site/pages/contact.html, site/pages/weddings.html) + concurrent-writer artifacts (`* 2`/`* 3` dups, AGENTS.md, TODO.md.tmp.*, rancho-moonrise-assets/, duplicated images) intentionally NOT staged. Committed by explicit path only — no `git add -A`. No site/pages/* edits, no schema touch, no sitemap touch. No SEO task IDs (s1–s8) touched. Run-log: `run-logs/2026-06-13-seo.md`.

## 2026-06-12 (rancho-site-daily) — 13th consecutive cruise-control firing, no shipped edits

- 13th consecutive `rancho-site-daily` cruise-control firing. No shipped edits. CreativeWork-property enrichment arc on the 17-post BlogPosting JSON-LD cluster closed 5/26 — no surgical autonomous slot remains. Both pre-scoped candidates (Person-author research, AggregateRating-on-BlogPosting) deferred again: one surfaces a new NEEDS ADAM, the other is a 17-file cluster diff against a workstream GOALS.md still pauses.
- **Re-Verify Gate (live, 7/7 still_true, 0 resolved):** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 43262` ≈ 12.0 hr); www 308 → apex; sitemap / corp-retreats / safari-tents / blog all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 17 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026). 0 stale claims → 0 done-log entries.
- GOALS.md still week-of-5/18 ("No Rancho Moonrise active work — paused"; 6/6 freeze-lift was styermortgage.com only). Indexing claims (`/corporate-retreats/`, `/safari-tents-near-austin/` uncrawled) owned by 6/8 competitive-weekly — not re-litigated. Review/GBP claims owned by review-monitor / gbp-weekly — not touched.
- Pre-existing prior-session changes (api/inquiry.js, site/css/styles.css, site/js/main.js, site/pages/contact.html, site/pages/weddings.html) + concurrent-writer artifacts (`* 2`/`* 3` dups, AGENTS.md, TODO.md.tmp.*, rancho-moonrise-assets/, duplicated images) intentionally NOT staged. Committed by explicit path only — no `git add -A`. No site/pages/* edits, no schema touch, no sitemap touch. No SEO task IDs (s1–s8) touched. Run-log: `run-logs/2026-06-12-seo.md`.
...
- **Re-Verify Gate (live, 7/7 still_true, 0 resolved):** apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 14746` ≈ 4.1 hr); www 308 → apex; sitemap.xml 200; `/corporate-retreats/` 200; `/safari-tents-near-austin/` 200; `/blog/` 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 16 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026).
- CreativeWork-property enrichment arc on the 17-post BlogPosting JSON-LD cluster closed 5/26 — no surgical autonomous slot remains. Person-author research + AggregateRating-on-BlogPosting both deferred again (noise / 17-file diff against a paused workstream). Indexing claims owned by 6/8 competitive-weekly, not re-litigated.
- No `site/` edits, no schema edits, no sitemap touch. Pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged; committed by explicit path only (no `git add -A`). Run-log: `run-logs/2026-06-11-seo.md`. No SEO task IDs (s1–s8) touched, no done-log entry.
...
- **Re-Verify Gate / auto-resolution:** Camp Lucy (`camplucy.com/meetings-events/corporate-packages`) prior claim "NEW on corporate-retreat top 10" (6/1) no longer true — page still live + selling corporate packages, but OFF the corporate-retreat SERP after exactly one week. SERP-presence drop, not page death. `camp-lucy-corporate-serp — RESOLVED` line appended to `rancho-done-log.md`.
- **CHANGED claims:** (1) Lucky Arrow ResortPass listing added a new daily Pool window (12:00pm–10:00pm) on top of the existing Fri–Sun bar/food (2–6pm) — still dormant, still 0 products. (2) Walden Retreats cleaned up the "four accommodation types / three named" copy inconsistency flagged 6/1 — now cleanly three named variants; unit count still 15.
- **New SERP churn:** glamping-near-austin dropped Camposanto ATX + The Yurtopian, added Green Acres Glamping (#2) + a Texas Parks & Wildlife guide, Hipcamp listicle slid #1→#4 (Udoscape #1). Corporate added Peaceful Waters Retreat Center + 7744 Ranch; Yurtopian off 4th consecutive week; Element Ranch + Crystal Creek hold. Spoon Mountain new entrant on safari-tent SERP (~#5).
- **Spoon Mountain reframe:** weekend-getaway guide page still OUT of broad `weekend getaway near austin glamping` top 10 (2nd consecutive week) BUT ranks **#1 for the narrower "romantic weekend getaways near austin."** Page healthy/indexed — durable on precise long-tail intent, doesn't compete on broad head term. Reframes the 4-week in/out oscillation. Does NOT escalate Rancho blog-pipeline restart.
- **Still true:** Rancho ResortPass dormant (drift now 21d since 5/18); Hipcamp curated 20 **identical to 6/1** (zero rotation — first non-rotating read; Lucky Arrow #8, Rancho absent 4th read); Safari for the Soul count still ambiguous (Sabi Nights page titled "Yurt" vs body "safari tent"); Yurtopian 10 yurts; Retreat on the Hill 11 units; Rancho 0/5 non-brand + #1 brand; `/corporate-retreats/` 38d + `/safari-tents-near-austin/` 43d still uncrawled.
- Outputs: `site/competitive-intelligence.md` rewritten for June 8; Intel tab card dual-written to `site/improvement-plan.html` (sovereign) + `client-ops/clients/rancho-moonrise/improvement-plan.html` (mirror), cards verified byte-identical. CONTEXT.md + TODO.md refreshed. Live liveness: apex 200 + `server: Vercel` + `x-vercel-cache: HIT`; www 308 → apex; sitemap + corp-retreats + safari-tents 200.
...
- **Hipcamp WebFetch FAILED 3rd consecutive run — NEW BLOCKER `hipcamp-direct-fetch` LOGGED.** RUN_041 + RUN_042 returned a Texas glamping guide list page; RUN_043 returned "Loading..." (JS-render incomplete for the fetcher). Different failure modes, same net result: zero Rancho-attributed content extractable. Per `BLOCKERS.md` consecutive-failure rule, threshold crossed (2→3). Hipcamp count/rating/voice-violations now STALE:2026-05-26 (last successful scrape). Resolution path: Apify or headless-render scraper, Adam manual spot-check, or quarterly downgrade (same as Hotels.com / The Knot).
- **Knot Haylee L. reviewer name RETURNS to snippet** — name + body both surface in Rancho-attributed snippet today after 1-run absence in RUN_042. 4-of-5-run pattern with name (RUN_039/040/041/043 yes; RUN_042 no). Visibility re-widens vs RUN_042. Carry-forward URGENT day 16: Cassie Google 5★ + Haylee Knot 1★ (~97 days unreplied) both still unposted.
- **Google WebSearch snippet returns NULL** — 6-run history now 126→175→null→126→126→null (4 distinct states). Snippet baseline NOT reforming; confirmed unreliable. Live=130/4.9★ from RUN_034 Chrome read remains authoritative (15 days old).
- **TripAdvisor STABLE at $63-$181 — 3rd consecutive run** at same range (28-run net floor -$14 / ceiling 0); Travelers' Choice NOT property-attributed (20th consecutive). Expedia 8.0 INLINE 13th consecutive run. Facebook 5/100% INLINE 24th consecutive run. The Knot direct WebFetch not attempted (BLOCKER 9-run no-attempt cycle). Hotels.com BLOCKER 42nd. Airbnb 403 BLOCKER 42nd.
- **Re-Verify Gate**: 10 prior live claims re-verified — 8 still_true, 1 STATE CHANGE (Google snippet null), 1 STALE-promoted-to-BLOCKER (Hipcamp). 0 resolved → no done-log writes.
- **Cruise-control firing-pattern**: today is the first calendar day with THREE Rancho task firings (site-daily AM + review-monitor 06:30 + content-weekly PM) across 3 distinct task IDs. **11th overall Rancho task firing across 4 distinct IDs since GOALS.md week-of-5/18 pause.** TODO.md NEEDS ADAM #0 strengthening deferred to content-weekly's commit (already 10 → that handler will catch this); review-monitor scope stays focused on review-state.
- **Files written**: `tasks/review-monitor/raw/2026-06-03/run-summary.md`, `brand/review-aggregate.json` (post-fresh-scrape, allowed by hard rule), `site/admin/dashboard-state.json`, `tasks/review-monitor/session-log.md` (RUN_043 appended), `tasks/review-monitor/BLOCKERS.md` (new BLOCKER entry), `run-logs/2026-06-03-review-monitor.md`, CHANGELOG.md (this entry).
- **CONTEXT.md NOT updated** — file already at 157 lines (over the 150-line cap); RUN_042 cruise-control pattern of leaving "Last Worked On" to the most recent SEO/content-weekly run preserved. Material state changes documented in `brand/review-aggregate.json` + `site/admin/dashboard-state.json` + session-log + this CHANGELOG entry.
- **Pre-existing uncommitted prior-session changes** (api/inquiry.js, styles.css, main.js, weddings.html, contact.html) intentionally NOT staged (5/7+ convention holds).
- **No review drafts produced this run** — no new reviews on any monitorable platform. Both RUN_034 carry-forward drafts remain at `brand/review-reports/2026-05-19-review-report.md`.
...
- **Re-Verify Gate (live, 9/9 still_true, 0 resolved)**:
  ...
  - 3-page BlogPosting schema spot-check (glamping-near-austin-texas, corporate-retreat-near-austin, wedding-venues-near-austin) — CreativeWork-property enrichment arc from 5/17 → 5/26 intact on live apex **8 days post-deploy**: `wordCount` (1005/1106/1256), `articleSection` (Glamping/Corporate Retreats/Weddings), `copyrightYear: 2026`, `dateModified: "2026-05-26"`. (Lighter check than 6/1 — verified glamping-near-austin-texas on all 6 properties as cluster baseline proxy.) No schema drift.
...
- **Pre-existing uncommitted prior-session changes** (api/inquiry.js, styles.css, main.js, weddings.html, contact.html) intentionally NOT staged (5/7+ convention holds). iCloud `* 2` / `* 3` duplicates, `rancho-moonrise-assets/`, `youtube-uploads/`, `tasks/review-monitor/raw/2026-05-27/`, `AGENTS.md` + `AGENTS 2.md`, and `run-logs/notebooklm-sync*.md` duplicates are all untouched (not site-daily's responsibility).
- **Run-log**: `run-logs/2026-06-03-seo.md`.
- **Improvement-plan mapping**: no SEO task IDs (s1–s8) touched. No done-log entry. No PROGRESS bumps.

## 2026-06-02 (rancho-site-daily) — Quiet maintenance run; no shipped edits. **4th `rancho-site-daily` firing against the cruise-control gate; 8th overall Rancho task firing across 4 distinct IDs.**

- **GOALS.md gate (same as 5/28 + 5/29 + 6/1)**: week of 2026-05-18 keeps Rancho on cruise control. `rancho-site-daily` not in the explicit pause-list. Task ran in maintenance mode only — re-verify gate + state spot-checks. The 30-second GOALS.md pause-list fix is now **6 days overdue** (NEEDS ADAM #0 first surfaced 5/27).
- **Cruise-control firing-pattern continues**: `rancho-site-daily` specifically is now at its 4th firing in 6 days (5/28 + 5/29 + 6/1 + 6/2). TODO.md NEEDS ADAM #0 strengthened from 7 firings / 4 IDs to **8 firings / 4 IDs**.
- **No schema edits.** Surgical-runway exhaustion from 5/26 holds. Pre-scoped next candidates (Person-author research, AggregateRating-on-BlogPosting cluster bump) remain deferred under cruise control — surfacing more NEEDS ADAM items or shipping cluster-wide edits against a paused workstream adds noise, not signal. Same call as 5/28, 5/29, and 6/1 maintenance runs.
- **No new NEEDS ADAM items.** Adam can't act on Rancho items this week per GOALS.md.
- **Re-Verify Gate (live, 9/9 still_true, 0 resolved)**:
  - 6 live URL checks: apex 200 + `server: Vercel` + `x-vercel-cache: HIT`; www 308 → apex; sitemap 200; `/corporate-retreats/` 200; `/safari-tents-near-austin/` 200; `/blog/` 200.
  - 3-page BlogPosting schema spot-check (glamping-near-austin-texas, corporate-retreat-near-austin, wedding-venues-near-austin) — CreativeWork-property enrichment arc from 5/17 → 5/26 intact on live apex **7 days post-deploy**: `wordCount` (1005/1106/1256), `articleSection` (Glamping/Corporate Retreats/Weddings), `copyrightYear: 2026`, `dateModified: "2026-05-26"`. (Lighter check than 6/1 — verified glamping-near-austin-texas on all 6 properties as cluster baseline proxy; 5/29 + 6/1 already verified the full 7-property set on all 3 pages with no drift.) No schema drift.
- **Findings owned by sister tasks (skipped per Re-Verify Gate ownership table)**: Google review reply state, GBP cadence, review counts (Google/TripAdvisor/Hipcamp/Expedia/Facebook), SERP positions for tracked keywords. Those belong to `rancho-review-monitor` and `rancho-competitive-weekly`.
- **Pre-existing uncommitted prior-session changes** (api/inquiry.js, styles.css, main.js, weddings.html, contact.html, run-logs/notebooklm-sync.md) intentionally NOT staged (5/7+ convention holds). Notebooklm-sync state JSON, iCloud `* 2` / `* 3` duplicates, `rancho-moonrise-assets/`, `youtube-uploads/`, `tasks/review-monitor/raw/2026-05-27/`, and `AGENTS.md` + `AGENTS 2.md` are all untouched (not site-daily's responsibility).
- **Run-log**: `run-logs/2026-06-02-seo.md`.
- **Improvement-plan mapping**: no SEO task IDs (s1–s8) touched. No done-log entry. No PROGRESS bumps.
...
- **Why `en-US` not `en`**: BCP 47 region tag disambiguates from `en-GB`/`en-AU` for international AI engines (Perplexity, Bing). Bing in particular weights regional language variants more heavily for SERP geo-targeting.
- **Metadata refresh**: `BlogPosting.dateModified` 2026-05-23 → 2026-05-24 across all 17; sitemap `<lastmod>` synced for all 17 blog URLs.
- **Validation**: `npm run validate:site` passes. All 17 BlogPosting JSON-LD blocks parse-valid via `python3 json.loads`. Every block now carries `"inLanguage": "en-US"` and `"dateModified": "2026-05-24"`.
- **Diff scope**: 18 files (17 blog posts + sitemap.xml), 68 insertions / 51 deletions. Uniform +31 bytes per blog post (exactly one new property line). Pre-existing prior-session changes in `api/inquiry.js`, `site/css/styles.css`, `site/js/main.js`, `site/pages/contact.html`, `site/pages/weddings.html` intentionally NOT staged (5/7–5/23 convention).
- **Improvement-plan mapping**: does NOT map to any of the 8 named SEO task IDs (s1–s8). No done-log entry.
- **Re-Verify Gate (live)**: apex 200 + `server: Vercel`; www 308 → apex; sitemap 200; `/corporate-retreats/`, `/safari-tents-near-austin/`, `/blog/` all 200. 6/6 verified claims still_true; 0 stale claims auto-resolved.
- **Next slot pre-scoped**: `isAccessibleForFree: true` on BlogPosting JSON-LD (surgical, autonomous, cluster-wide). Backup: `copyrightYear` + `copyrightHolder`.
...
- **Why ItemList not HowTo.** HowTo requires procedural content where step N+1 depends on having done step N — this page is a roundup of 5 independent destinations, not a procedure. `ItemList` is schema.org's first-class type for curated/ranked list content and is Google's recommended schema for listicle pages.
- **Why this candidate.** Yesterday's competitive-weekly logged Spoon Mountain's new "Ultimate 2026 Guide" surfacing on the exact SERP this post targets — leading indicator that the weekend-getaway cluster is being actively contested. Page already had clean BlogPosting + BreadcrumbList scaffolding, so marginal cost was one new block. Survey of remaining blog posts: 0 HowTo-fit candidates (only `corporate-retreat-planning-guide-texas` had explicit "Step N" structure, shipped 5/11); 3 ItemList-fit candidates remain (`weekend-getaways` shipped today, `things-to-do-manor-tx` and `things-to-do-near-austin-with-kids` pre-scoped for next slots).
- **Item content.** Each `ListItem.description` is a 1–2 sentence distillation of the corresponding body section, not a copy of body content. Distilled rather than mirrored so the schema text stands on its own when an AI engine extracts an individual item in isolation. Position 1 (Rancho Moonrise) has `url: https://ranchomoonrise.com/`; positions 2–5 are name+description only — no fake URLs claimed for properties we don't own. `itemListOrder` intentionally omitted (the numbering is editorial/geographic, not a competitive best-to-worst ranking; omitting is the honest framing).
- **Metadata refresh.** `BlogPosting.dateModified` bumped 2026-04-11 → 2026-05-12 since the page-level schema actually changed today. Sitemap `lastmod` for the same URL bumped 2026-05-10 → 2026-05-12.
- **Validation.** `npm run validate:site` passes. All 3 JSON-LD blocks parse via `python3 json.loads`: BlogPosting (dateModified=2026-05-12), ItemList (numberOfItems=5 matches itemListElement.length=5), BreadcrumbList (3 crumbs).
- **Diff.** 2 files, 46 insertions, 1 deletion — surgical. Pre-existing uncommitted prior-session changes in styles.css/main.js/weddings.html intentionally NOT staged (5/7, 5/8, 5/9, 5/10, 5/11 convention).
- **Re-Verify Gate (live).** apex 200 + `server: Vercel` + `x-vercel-cache: HIT`; www 308 → apex; sitemap 200; target page `/blog/weekend-getaways-near-austin/` 200; all 9 critical landing pages 200; yesterday's target `/blog/corporate-retreat-planning-guide-texas/` 200. All 13 verified claims still_true; 0 stale claims auto-resolved.
...
- **New finding — Spoon Mountain content marketing.** Shipped `/travel-to-wimberley/romantic-weekend-getaways-near-austin/` — "Ultimate 2026 Guide" with 5 destinations, 3-day Wimberley itinerary, budget breakdown. Now surfacing in "weekend getaway near austin glamping" SERP top 10. First tracked competitor investing in destination-guide research content.
- **New finding — Loving Heart Retreats** surfacing in "safari tent austin texas" SERP description (25 ac, 12 min from Marble Falls). Domain has TLS cert mismatch, not fetchable today. Adds to safari-tent cluster without displacing anyone.
- **New finding — Hipcamp curation gap.** Hipcamp's editorial "20 Best Luxury Glamping Sites 2026 Near Austin" landing page does NOT include Rancho Moonrise despite an active listing. Top 5 are urban/airstream/East Austin properties (Urban Hideout, Cozy Cactus, Breathe Deeply, River Forest Haven, Texas Music River Ranch). Discoverability gap worth probing.
- **New finding — Hotels.com listing copy.** ho2867109568 surfaces "20 luxury cabins and safari tents that can accommodate up to 50 guests" in brand SERP snippet. The Vercel site itself does NOT contain this copy. Per VOICE-GUIDE.md no specific unit counts — Hotels.com listing needs an edit. Hotels.com page timed out today, follow-up next run.
- **SERP rankings unchanged.** Glamping near austin tx (Udoscape #1, no Rancho), safari tent austin texas (Safari for the Soul #1, no Rancho), corporate retreat near austin texas (Wilder/Element Ranch top 5, no Rancho — `/corporate-retreats/` 10 days post-launch still uncrawled), weekend getaway near austin glamping (Udoscape/Cameron Ranch top 5, no Rancho — but Spoon Mountain's new content page now surfacing), wedding venue manor tx (Honeysuckle Ranch dominant, no Rancho). Brand SERP: ranchomoonrise.com #1 (Vercel apex, 200 OK, `server: Vercel`, `x-vercel-cache: HIT`).
- **Action items (refreshed).** Priority 1: GSC sitemap submit (Adam — 3rd consecutive week). Priority 2: ResortPass decision (urgency framing fully dead — pick a side and stop drifting). Priority 3: Hotels.com listing copy fix (Adam ~15 min). Priority 4: Blog pipeline restart (Adam + Ashley check-in — Spoon Mountain's move is a leading indicator).
- **Output**: `site/competitive-intelligence.md` rewritten for May 11; Intel tab card dual-written to `site/improvement-plan.html` (sovereign) and `client-ops/clients/rancho-moonrise/improvement-plan.html` (mirror).

## 2026-05-11 (rancho-review-monitor) — RUN_025: Google snippet observability dip (9-run stable stretch BROKEN), TA ceiling -$1 partial reversal, Expedia 8.0 inline 4th run

- rancho-review-monitor: scraped 5 platforms across 2 live WebFetch (TripAdvisor, Hipcamp) + 3 WebSearch confirmations (Google, Expedia/Hotels.com, Facebook); 0 stale claims auto-resolved; 0 response drafts written; 3 BLOCKERS still open at 25 consecutive runs (google-reviews-count, hotels-com-direct-fetch, airbnb-listing-existence). **Google count snippet ABSENT across three WebSearch queries** — RUN_016–024 9-run stable stretch at 126 @ 4.9★ is broken; 25-run pattern is now 126→175→126→175→126→126→126→126→126→126→126→126→126→ABSENT (identical shape to the RUN_021 one-run dip). True count again unknown from snippets alone. TripAdvisor price drift partially reversed — $66-$179 → $66-$178 (-$1 ceiling, floor unchanged — partial unwind of RUN_024's $4 ceiling jump). Travelers' Choice text continues NOT attributed to property (3rd run after RUN_023 reframing). Expedia 8.0 inline 4th consecutive run — verbatim "solid guest review rating of 8.0" (same exact phrasing as RUN_022/023/024). Facebook 5/100% recommend stable 7th consecutive run after RUN_018 dip. Hipcamp identical to RUN_023/024 (0 reviews, 1 booking singular, joined March 2024). The Knot RE-SURFACED with explicit count detail (4.5★ / 8 reviews — same as RUN_012/013 last hard-data; no growth in 13 runs). Aggregate JSON + dashboard-state JSON + session-log + CONTEXT 'Last worked on' refreshed.

## 2026-05-10 (rancho-site-daily) — Internal-linking close-out: 3 lightly-linked posts → 4 inbound each

- **Continuation of 5/9 orphan pass.** Yesterday closed 4 critically-orphaned blog posts (1 → 4 inbound). Today closes the 3 lightly-linked posts that were flagged as the next slot's autonomous candidate: `austin-bachelorette-ranch-vs-bar-crawl`, `glamping-vs-camping`, `summer-glamping-near-austin` (each at 2 inbound: blog index + 1 cluster page).
- **Edits — 6 surgical `<li>` inserts across 5 host-page Related Reading lists.** Each new item placed before the trailing "More from the Rancho Moonrise Blog" item where present, preserving the final-CTA pattern. Topical pairings: `weekend-getaways-near-austin.html` ← bachelorette-vs-bar + glamping-vs-camping (decision-aid + sibling weekend content); `glamping-near-austin-texas.html` ← bachelorette-vs-bar (bachelorette glamping is a real SERP query); `safari-tents-near-austin.html` ← glamping-vs-camping (the question hits at the conversion-decision moment); `events.html` ← summer-glamping (peak event season); `weddings.html` ← summer-glamping (peak wedding season).
- **Result.** All 3 posts went from 2 → 4 inbound links. Cluster now has zero blog posts below the 4-inbound baseline established by `wedding-venues-near-austin`.
- **Sitemap freshness.** 5 lastmod entries bumped to 2026-05-10: `/weddings/` (2026-05-06), `/events/` (2026-05-09), `/blog/glamping-near-austin-texas/` (2026-05-04), `/blog/weekend-getaways-near-austin/` (2026-05-09), `/safari-tents-near-austin/` (2026-04-26 — caught a 2-week-stale entry).
- **Validation.** `npm run validate:site` passes.
- **Diff.** 6 files, 11 insertions, 5 deletions (sitemap date replacements) — surgical. Pre-existing uncommitted prior-session changes in styles.css/main.js (and the accommodations-section delete in weddings.html) intentionally NOT staged; weddings.html staged via focused patch on the 1-line Related Reading insert (5/7, 5/8, 5/9 convention).
- **Re-Verify Gate (live).** apex 200 + `server: Vercel` + `x-vercel-cache: HIT`; www 308 → apex; sitemap 200; `/corporate-retreats/`, `/safari-tents-near-austin/`, `/faqs/`, `/accommodations/`, `/host-your-event/`, `/events/`, `/weddings/`, `/contact/`, all 3 target blog posts, and `/blog/corporate-retreat-planning-guide-texas/` all 200. All DNS/canonical/sitemap claims still_true; 0 stale claims auto-resolved.
- **Why this run over Article/HowTo schema enrichment.** Internal-linking close-out (a) finishes a multi-day workstream within one slot rather than half-shipping two; (b) brings the entire blog cluster to a clean inbound-link floor before the next non-linking lever; (c) leaves Article/HowTo schema on `corporate-retreat-planning-guide-texas` as a clean, well-scoped next slot.
...
- **Edits — 12 surgical `<li>` inserts across 7 host-page Related Reading lists.** Each new item placed before the trailing "More from the Rancho Moonrise Blog" item to preserve that as the final CTA. Topical pairings: events.html ← yoga + birthday + mothers-day; weekend-getaways-near-austin.html ← yoga + kids + birthday; things-to-do-manor-tx.html ← kids + mothers-day; bachelorette-party-austin-texas.html ← birthday; corporate-retreat-near-austin.html ← yoga; summer-glamping-near-austin.html ← kids; things-to-do-near-austin-with-kids.html ← mothers-day. Link text mirrors each orphan's H1.
- **Result.** All 4 orphans went from 1 → 4 inbound links (4x improvement). Now match the inbound-count baseline of `wedding-venues-near-austin` (4) and exceed `ranch-wedding-texas` (3).
- **Sitemap freshness.** 7 lastmod entries bumped to 2026-05-09 (`/events/`, plus 6 blog post URLs).
- **Validation.** `npm run validate:site` passes.
- **Diff.** 8 files changed, 12 line insertions, 7 sitemap date changes — surgical. Pre-existing uncommitted prior-session changes in styles.css/main.js/weddings.html intentionally NOT staged (5/7, 5/8 convention).
- **Re-Verify Gate (live).** apex 200 + `server: Vercel` + `x-vercel-cache: HIT`; www 308 → apex; `/sitemap.xml`, `/corporate-retreats/`, `/safari-tents-near-austin/`, `/faqs/`, `/accommodations/`, `/host-your-event/`, `/events/`, `/weddings/`, `/contact/` all 200. All DNS/canonical/sitemap claims still_true; 0 stale claims auto-resolved.
- **Why this run over alternatives.** CONTEXT.md "Next SEO run (May 9+)" listed 5 autonomous candidates. Picked the orphaned-blog-post audit because it was (a) fully autonomous, (b) trivially auditable via grep, (c) addressed a measurable site-architecture defect (blog index as sole PageRank source for 4 posts), and (d) pre-positions the site for the eventual GSC reindex. AEO baseline (s7) deferred — low signal before apex indexing. Article/HowTo enrichment on planning guide queued for next slot.
- **Run-log.** `run-logs/2026-05-09-seo.md` with full audit table and topical-pairing rationale.
...
- **Schema edit (`site/pages/faqs.html`).** One Q&A appended to the FAQPage `mainEntity` array: "Is Rancho Moonrise a working ranch?" → "Yes — Rancho Moonrise is a working ranch with livestock and wildlife on the property. No hunting is allowed. Guests are asked to be mindful of animals and follow posted guidelines." Mirrors accordion answer with minor third-person rephrase ("Guests are asked" rather than "Please be"), consistent with the rest of the schema's voice.
- **Voice compliance.** No banned terms; reuses existing body-copy framing about "working ranch with livestock and wildlife"; no new claims added to the page.
- **Sitemap freshness.** `/faqs/` lastmod bumped 2026-05-07 → 2026-05-08.
- **Validation.** `npm run validate:site` passes. All 4 JSON-LD blocks on `/faqs/` re-parse via `python3 json.loads`: FAQPage (now 18 items), BreadcrumbList, LocalBusiness, WebPage. Question-name divergence between schema (long-form) and accordion (short-form) intentionally left as-is — the answer text is what gets cited, the name is metadata.
- **Diff.** 2 files, 6 insertions, 1 deletion — surgical. Pre-existing uncommitted prior-session changes in styles.css/main.js/weddings.html were intentionally NOT staged (same as 5/7 run).
- **Re-Verify Gate (live).** apex 200 + `server: Vercel` + `x-vercel-cache: HIT`; www 308 → apex; `/sitemap.xml`, `/corporate-retreats/`, `/safari-tents-near-austin/`, `/faqs/`, `/accommodations/`, `/host-your-event/`, `/events/`, `/weddings/`, `/contact/` all 200. All DNS/canonical/sitemap claims still_true. **1 prior persistent claim auto-resolved** during the gate: yesterday's TODO claim "no FAQPage schema block on /faqs/" → false-positive, FAQPage already present, schema/accordion parity now closed.
- **Why this run over AEO baseline (rancho-seo-s7).** AEO baseline measurement requires querying live AI engines for the 10 target keywords — heavy run, low signal before GSC apex indexing happens (NEEDS ADAM). Schema parity is autonomous, immediately testable, and pre-positions one more rich-result lever for the eventual reindex. Saves the AEO baseline for the run after Adam submits the sitemap.
...
- **State correction (material):** Lucky Arrow's ResortPass dormancy did **not** end on May 1 as their site copy promised. Three days past the projected reactivation, the listing still shows "no active products at the moment" via WebFetch. The April 27 "decide within 48 hours" framing was wrong — window EXTENDED, not closed. Submission decision is now a clean either/or, not a sprint.
- **New SERP entrant:** Green Acres ATX (greenacresatx.com, Elgin, ~25 min from Manor) surfacing in "glamping near austin tx" top 10. 8 multi-format units (Lodge, Spartan trailer, Airstream, 4 yurts, full compound rental). Press in U.S. News, Dwell, Apartment Therapy, Austin Monthly. Closest geographic competitor surfaced to date.
- **Snippet tightening:** Walden Retreats SERP description now reads "15 rooms of luxury safari style glamping tents, including 7 suites and 8 studios" — most explicit unit-count framing they've surfaced in any recent run; SERP-dominant alongside Safari for the Soul on safari tent queries.
- **Snippet tightening (subject property):** Wahwahtaysee Resort newly prominent in safari tent SERP descriptions (7 tents + 3 cabins, 100+ ac San Marcos River, 50 mi south).
- **Subject-property side (no agent action this run, but bears on positioning):** DNS cutover live 4/30 + `/corporate-retreats/` shipped 5/1 + internal-linking pass 5/2 + `/events/` AggregateRating 5/3 = the build-side competitive gap (corporate retreats page) is closed. Bottleneck has moved from "DNS blocking everything" to "Google has not yet crawled the apex." GSC sitemap submission is now the highest-leverage SEO action.
- **Action items:** (1) GSC sitemap submission promoted to Priority 1 in CONTEXT.md NEEDS ADAM. (2) ResortPass extended-window decision (submit-or-kill) added to NEEDS ADAM. (3) Track Green Acres ATX as the closest geographic competitor — their editorial press footprint suggests April 6's press-pitch action item needs a different angle.
- **Files:** `site/competitive-intelligence.md` (full report rewritten for May 4), `site/improvement-plan.html` (Intel tab card inserted above NEWEST CARDS marker), `client-ops/clients/rancho-moonrise/improvement-plan.html` (mirror of same Intel tab card), `CONTEXT.md`, `CHANGELOG.md`, `TODO.md`.
...
## 2026-05-04 — Review Monitor RUN_018

- **rancho-review-monitor RUN_018:** Quiet run. Google WebSearch snippet at 126 @ 4.9★ for the third consecutive run (RUN_016/017/018) — longest stable stretch since the ping-pong started at RUN_011 (history now 126→175→126→175→126→126→126), but three earlier flips on consecutive runs keep this lower-confidence stable rather than authoritative. TripAdvisor live-scraped 0 reviews / unclaimed / Travelers' Choice (price drifted $74-$175 → $73-$175, -$1 floor, ceiling steady at $175; 7-run drift floor $77→$73 / ceiling $181→$175 is now a pattern). Hipcamp live-scraped 0 reviews / 1 booking / joined Mar 2024 — unchanged. Expedia 8.0 still surfacing on Hotels.com listing ho2867109568 (snippet noted "last rating from December 2025"). Facebook observability dip: WebSearch returned the FB page link but did NOT surface the 5/100% recommend snippet inline — first dip since RUN_010, no new state. 3 BLOCKERS ongoing (Google JS-blocked, Hotels.com timeout, Airbnb 403) — all at 18 consecutive runs. No review-related done-log entries since RUN_011 — `rancho-review-replies` RESOLVED 2026-04-15 22:12 remains last review action; unreplied=0 maintained. Dashboard status: ok.

## 2026-05-12 — Review Monitor RUN_026

- **rancho-review-monitor RUN_026:** Quiet run. Google WebSearch snippet **re-surfaced at 126 @ 4.9★** after RUN_025's all-three-queries-ABSENT dip — confirmed as one-run observability gap, identical shape to RUN_021. Pattern across 26 runs now 126→175→126→175→126→126→126→126→126→126→126→126→126→ABSENT→126 (effectively 10-of-11 stable with one ABSENT mid-stretch). TripAdvisor + Hipcamp live-confirmed 0 reviews (TripAdvisor still unclaimed; Travelers' Choice text NOT property-attributed for 4th consecutive run). **TripAdvisor price UNCHANGED at $66-$178 — first no-drift run since RUN_022** after 4 consecutive drift runs RUN_021–025. Facebook 5/100% recommend snippet stable for 8th consecutive run. Expedia 8.0 rating value INLINE for 5th consecutive run. Hipcamp identical to RUN_023/024/025 (1 booking, joined March 2024). 3 BLOCKERS ongoing (google-reviews-count, hotels-com-direct-fetch, airbnb-listing-existence) — counters bumped to 26. No review-related done-log entries since RUN_011 — `rancho-review-replies` RESOLVED 2026-04-15 22:12 remains last review action; unreplied=0 maintained. Dashboard status: ok.

## 2026-05-15 — Review Monitor RUN_029 (catch-up + quiet hold)

- **rancho-review-monitor RUN_029:** Catch-up + Google snippet 4th consecutive post-dip confirmation. Yesterday's RUN_028 wrote `brand/review-aggregate.json` + `site/admin/dashboard-state.json` but was never committed — no `rancho-review-monitor` commit appears between RUN_027 (5/13 21cc5bc) and today; today's run supersedes those orphaned values. Run numbering jumps 027 → 029. **Google count snippet HELD at 126 @ 4.9★** — first WebSearch query surfaced "4.9 rating with 126 reviews" inline (4th consecutive run post the RUN_025 one-run ABSENT). Pattern across 29 runs: 126→175→126→175→126→126→126→126→126→126→126→126→126→ABSENT→126→126→126→126 — 13-of-14 stable; recovery is four-for-four. TripAdvisor live-scraped: 0 reviews, unclaimed; **price REVERTED $65-$178 → $65-$175** (ceiling -$3, fully reverses RUN_028's one-run ceiling +$3 bump; back to RUN_027 levels). 18-run net drift floor $77→$65 / ceiling $181→$175. Travelers' Choice text continues NOT property-attributed — 7th consecutive run. **Hipcamp live-scrape FAILED today** — WebFetch returned "Loading..." on both attempts (JS render gap, first such failure since live-scraping became routine; aggregate retains last successful RUN_028 values, tagged STALE:2026-05-14, scrape_failure_count=1). **Expedia 8.0 rating value NOT inline this run** — first absence after SEVEN consecutive inline confirmations (RUN_022-028); mirrors the RUN_021 one-run observability dip. Hotels.com listing ho2867109568 still active. Facebook 5/100% recommend snippet stable — 11th consecutive run after RUN_018 dip. The Knot listing in result list but no count detail re-surfaced — 4 consecutive runs without count. Airbnb 403 BLOCKER ongoing (29th). 3 BLOCKERS at 29 runs (Google count, Hotels.com timeout, Airbnb 403). No review-related done-log entries since RUN_011 — `rancho-review-replies` RESOLVED 2026-04-15 22:12 remains last review action; unreplied=0 maintained. Dashboard status: ok.

## 2026-05-23 — Review Monitor RUN_037 (NEW BLOCKER + Hipcamp 1-run dip)

- **rancho-review-monitor RUN_037:** Fourth consecutive quiet platform sweep — no new reviews on any monitored platform. **NEW BLOCKER opened: `theknot-direct-fetch` — 3rd consecutive 60s timeout (RUN_035-037)**; logged to `tasks/review-monitor/BLOCKERS.md` with WebSearch as the documented working fallback. Two RUN_034 drafts still unposted day 5 (Cassie Butterfield Google 5★ ~day 7 since posting; Haylee L. The Knot 1★ ~day 93 unreplied); done-log shows no review-related entry since 2026-04-15 22:12. Google live=130/4.9★ carries from RUN_034 (Chrome-verified, now 4 days old); WebSearch snippet still 126 — snippet-vs-live lag confirmed 4 consecutive runs (delta=4). TripAdvisor live: 0/unclaimed, **price range $63-$179 IDENTICAL across RUN_035-036-037 — 3 consecutive runs no drift** (RUN_035's +$1 ceiling firmly holding). Travelers' Choice text NOT attributed 14th consecutive run. **Hipcamp live-scrape FAILED 1-run dip** — WebFetch returned only "Loading..." JS-render shell; 7-run clean streak (RUN_028 + RUN_030-036) broken; same RUN_029 pattern; not yet BLOCKER (threshold = 3 consecutive); WebSearch fallback confirms listing copy still carries '34-acre' + 'drink at the bar' violations. Expedia 8.0 INLINE 7th consecutive run. Facebook 5/100% INLINE 18th consecutive run. The Knot Haylee L. review text still surfaces via WebSearch confirming review remains live (~day 93 from 2026-02-26); no Rancho-attributed owner response indexed. Hotels.com BLOCKER 36th. Airbnb 403 BLOCKER 36th. **4 BLOCKERS** now ongoing (Google count, Hotels.com timeout, Airbnb 403, **Knot direct-fetch NEW**). Dashboard status: pending. Report: `brand/review-reports/2026-05-23-review-report.md`.

## 2026-05-24 — Review Monitor RUN_038 (TripAdvisor +$1 ceiling drift, Hipcamp 2nd dip)

- **rancho-review-monitor RUN_038:** Fifth consecutive quiet platform sweep — no new reviews on any monitored platform. Two RUN_034 drafts still unposted day 6 (Cassie Butterfield Google 5★ ~day 8 since posting; Haylee L. The Knot 1★ ~day 94 unreplied); done-log shows no review-related entry since 2026-04-15 22:12. **TripAdvisor +$1 ceiling drift**: $63-$179 (3 consecutive runs RUN_035-037) → **$63-$180** — first drift after RUN_035's +$1 ceiling held stable for 3 runs. Floor unchanged $63. 25-run net floor $77→$63 (-$14) / ceiling $181→$180 (-$1, narrowed from -$2 at RUN_037). Travelers' Choice text NOT attributed 15th consecutive run. **Hipcamp 2nd consecutive "Loading..." failure** (RUN_037 = 1st dip, RUN_038 = 2nd) — threshold = 3 consecutive; one more failure next run = BLOCKER. WebSearch confirms listing still indexed; '34-acre' + 'drink at the bar' voice/data violations carry. **The Knot direct fetch 4th consecutive 60s timeout** — BLOCKER `theknot-direct-fetch` (opened RUN_037) remains active; WebSearch fallback confirms Haylee L. review body text still surfaces ("neighboring property played extremely loud amplified music" + "venue has no ability to control" + "risk was never disclosed"); reviewer name "Haylee" only in unrelated context (Pham TikTok); no Rancho-attributed owner response indexed. Google live=130/4.9★ carries from RUN_034 (Chrome-verified, now 5 days old); WebSearch snippet still 126 — snippet-vs-live lag confirmed 5 consecutive runs (delta=4). Expedia 8.0 INLINE 8th consecutive run. Facebook 5/100% INLINE 19th consecutive run. Hotels.com BLOCKER 37th. Airbnb 403 BLOCKER 37th. **4 BLOCKERS** ongoing (Google count, Hotels.com timeout, Airbnb 403, Knot direct-fetch). Dashboard status: pending. Report: `brand/review-reports/2026-05-24-review-report.md`.

## 2026-06-01 — Review Monitor RUN_041 (TripAdvisor ceiling full round trip, Hipcamp 1st failure post-recovery)

- **rancho-review-monitor RUN_041:** Eighth consecutive quiet platform sweep — no new reviews on any monitored platform. Two RUN_034 drafts still unposted day 14 (Cassie Butterfield Google 5★ ~day 16 since posting; Haylee L. The Knot 1★ ~95 days unreplied); done-log shows no review-related entry since 2026-04-15 22:12. **TripAdvisor ceiling drift back to $181**: $63-$180 (3 consecutive RUN_038/039/040) → **$63-$181** — full round trip to RUN_001 baseline after the 3-run stabilization broke. 26-run net floor $77→$63 (-$14) / ceiling $181→$181 (0 net). Travelers' Choice text NOT attributed 18th consecutive run. **Hipcamp scrape FAILED 1st time after RUN_039/040 recoveries** — WebFetch resolved to a Texas glamping guide page (Best Dam Spot / Good Guad Land Co. / etc.) instead of the Rancho-specific listing; failure counter 0 → 1 (BLOCKER threshold 3); voice/data violations not re-verifiable today, carry from RUN_040. **Google WebSearch snippet returned to 126** — fourth distinct state in four consecutive runs (126 stable 12 runs → 175 RUN_039 → null RUN_040 → 126 RUN_041); snippet officially unreliable as authoritative count source. Live=130/4.9★ carries from RUN_034 Chrome read (now 13 days old). **The Knot direct fetch 7th consecutive 60s timeout** — BLOCKER `theknot-direct-fetch` (opened RUN_037) remains active; WebSearch fallback surfaces Haylee L. reviewer name + verbatim body in Rancho-attributed snippets for the 3rd consecutive run — bad-review search exposure widening week-over-week. Expedia 8.0 INLINE 11th consecutive run. Facebook 5/100% INLINE 22nd consecutive run. Hotels.com BLOCKER 40th. Airbnb 403 BLOCKER 40th. **4 BLOCKERS** ongoing (Google count, Hotels.com timeout, Airbnb 403, Knot direct-fetch). Dashboard status: pending. No new review-report produced (no new reviews to draft); both RUN_034 carry-forward drafts remain at `brand/review-reports/2026-05-19-review-report.md`.

## 2026-06-02 — Review Monitor RUN_042 (Hipcamp 2nd failure, TripAdvisor + Google snippet stable, Knot visibility narrowed)

- **rancho-review-monitor RUN_042:** Ninth consecutive quiet platform sweep — no new reviews on any monitored platform. Two RUN_034 drafts still unposted day 15 (Cassie Butterfield Google 5★ ~day 17 since posting; Haylee L. The Knot 1★ ~96 days unreplied); done-log shows no review-related entry since 2026-04-15 22:12. **Hipcamp scrape FAILED 2nd consecutive run** — WebFetch resolved to a Texas glamping guide page (Good Guad Land Co. / Best Dam Spot / Sparrow Bend River Retreat / etc.) for the second day in a row; failure counter 1 → 2 (BLOCKER threshold 3); **one more failure on RUN_043 = `hipcamp-direct-fetch` BLOCKER logged**. Voice/data violations not re-verifiable for 2 consecutive runs; carry from RUN_040. **TripAdvisor STABLE at $63-$181** — RUN_041's drift back to RUN_001 baseline holds 2 consecutive runs; 27-run net floor $77→$63 (-$14) / ceiling $181→$181 (0 net). Travelers' Choice text NOT attributed 19th consecutive run. **Google WebSearch snippet STABLE at 126** — 2 consecutive runs at 126 after RUN_039/040 divergence (126→175→null→126→126); tentative baseline reforming but snippet remains unreliable as authoritative source. Live=130/4.9★ carries from RUN_034 Chrome read (now 14 days old). **The Knot direct fetch not attempted** — BLOCKER `theknot-direct-fetch` (opened RUN_037) ongoing; 8 runs no-attempt cycle. WebSearch fallback surfaces Haylee L. review BODY in Rancho-attributed snippet but reviewer NAME dropped from today's snippet (was present RUN_039/040/041, 3 consec runs with name); visibility slightly narrowed vs RUN_041 but review still discoverable. Expedia 8.0 INLINE 12th consecutive run. Facebook 5/100% INLINE 23rd consecutive run. Hotels.com BLOCKER 41st. Airbnb 403 BLOCKER 41st. **4 BLOCKERS** ongoing (Google count, Hotels.com timeout, Airbnb 403, Knot direct-fetch). Dashboard status: pending. No new review-report produced (no new reviews to draft); both RUN_034 carry-forward drafts remain at `brand/review-reports/2026-05-19-review-report.md`. Cruise-control posture: CONTEXT.md "Last Worked On" left to today's earlier site-daily entry to minimize cruise-control noise during GOALS.md pause week.

## 2026-06-06 — SEO daily (7th site-daily cruise-control firing, quiet maintenance)

- **rancho-site-daily:** No shipped edits. 7th `rancho-site-daily` cruise-control firing (5/28, 5/29, 6/1, 6/2, 6/3, 6/4, 6/6). GOALS.md week-of-5/18 pause ("No Rancho Moonrise active work — paused") still active; CreativeWork-property enrichment arc on the 17-post BlogPosting JSON-LD cluster closed 5/26, no surgical autonomous slot remains, and both pre-scoped candidates (Person-author research / AggregateRating-on-BlogPosting) add noise not signal against a paused workstream. **Re-Verify Gate (live): 9/9 still_true, 0 resolved** — apex 200 + `server: Vercel` + `x-vercel-cache: HIT` (`age: 4622`); www 308 → apex; sitemap 200; `/corporate-retreats/`, `/safari-tents-near-austin/`, `/blog/` all 200; BlogPosting CreativeWork 6-property spot-check on `/blog/glamping-near-austin-texas/` intact 11 days post-5/26 deploy (dateModified 2026-05-26, wordCount 1005, articleSection "Glamping", inLanguage "en-US", isAccessibleForFree true, copyrightYear 2026). No SEO task IDs (s1–s8) touched, no done-log entry, no PROGRESS bumps. Pre-existing uncommitted prior-session changes (api/inquiry.js, styles.css, main.js, weddings.html, contact.html) intentionally NOT staged. TODO.md NEEDS ADAM #0 pause-list fix now 10 days overdue (5/27 surface). Run-log: `run-logs/2026-06-06-seo.md`.

## 2026-06-13 — Review Monitor RUN_050 (17th quiet sweep; Expedia snippet divergence; TripAdvisor ceiling oscillation)

- **rancho-review-monitor RUN_050:** Seventeenth consecutive quiet platform sweep — no new reviews on any monitored platform, no rating/count drops, no new ≤3★. Cruise-control maintenance run under GOALS.md week-of-5/18 broad Rancho pause (review-monitor not in the Scheduled-Tasks Pause List; monitoring ≠ active work). Two RUN_034 drafts still unposted day 26 in monitor (Cassie Butterfield Google 5★ ~day 28 since posting; Haylee L. The Knot 1★ 107 days unreplied); done-log shows no review-related entry since 2026-04-15 22:12. **Re-Verify Gate: 8/8 still_true, 0 resolved** (no done-log RESOLVED line). **TripAdvisor** DIRECT WebFetch — 0/unclaimed unchanged; price ceiling ticked **$180 → $179** after a 3-run hold at $64–$180 (RUN_047–049) — immaterial oscillation (algorithmic rate estimate, not a review). **Google** WebSearch snippet flipped back **175 → 126** + 4.9★ surfaced; oscillation history 126/175/null/126/175/126 — non-authoritative; live 130/4.9★ carries from RUN_034 Chrome read (25 days old, STALE). **Expedia snippet DIVERGENT this run** — a single sweep surfaced THREE conflicting values (**8.0 / 9.0 'Wonderful' / 8.6-out-of-10 'Excellent'**) across two distinct Expedia entities (`h89565924` + Hotels.com-shared `ho2867109568`); 8.0 anchor holds (surfaced in both queries, 20th consecutive inline run), 9.0/8.6 treated as snippet/listing-variant artifacts — **NO rating change asserted per the Re-Verify Gate**; first run multiple conflicting Expedia values appeared in one sweep, reliability downgraded (same treatment as the Google snippet). **Facebook** 5/100% INLINE 31st consecutive run. **The Knot** Haylee L. 1★ body still indexed in Rancho-attributed snippet via name-free query (name-surfacing not asserted, no owner-response indexed, 107 days unreplied); 8/4.5★ carried; direct fetch not attempted (BLOCKER `theknot-direct-fetch`, 16th no-attempt run). **Hipcamp** direct fetch not attempted (BLOCKER `hipcamp-direct-fetch`, 7th no-attempt-cycle run); listing still indexed via WebSearch; voice/data violations ('34-acre' vs VOICE-GUIDE 36; 'a bar') re-surfaced, carry STALE:2026-05-26. **Airbnb** 403 BLOCKER 49th no-attempt; **Hotels.com** timeout BLOCKER (counter holds 42, cruise-control). Dashboard status: `pending`. No new review-report produced (no new reviews to draft); both carry-forward drafts remain at `brand/review-reports/2026-05-19-review-report.md`. State updated: `brand/review-aggregate.json` + `site/admin/dashboard-state.json` + `run-logs/2026-06-13-review-monitor.md`. Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged.

## 2026-06-14 — Review Monitor RUN_051 (18th quiet sweep; Google snippet 2-run hold at 126; Expedia 8.0 anchor reconfirmed)

- **rancho-review-monitor RUN_051:** Eighteenth consecutive quiet platform sweep — no new reviews on any monitored platform, no rating/count drops, no new ≤3★. Cruise-control maintenance run under GOALS.md week-of-5/18 broad Rancho pause (review-monitor not in the Scheduled-Tasks Pause List; monitoring ≠ active work). Two RUN_034 drafts still unposted day 27 in monitor (Cassie Butterfield Google 5★; Haylee L. The Knot 1★ 108 days unreplied); done-log shows no review-related entry since 2026-04-15 22:12. **Re-Verify Gate: 8/8 still_true, 0 resolved** (no done-log RESOLVED line). **TripAdvisor** DIRECT WebFetch — 0/unclaimed unchanged; price floor ticked **$64 → $65** (ceiling holds $179) vs RUN_050's $64–$179 — immaterial oscillation (algorithmic rate estimate, not a review). **Google** WebSearch snippet **held at 126** + 4.9★ surfaced — first 2-run hold at 126 since the flip-flop began (history 126/175/null/126/175/126/126); non-authoritative; live 130/4.9★ carries from RUN_034 Chrome read (26 days old, STALE). **Expedia** 8.0 anchor **CONFIRMED** on targeted query (both Expedia + Hotels.com, 21st consecutive inline run); general query surfaced **9.0 'Wonderful'** (artifact, 2nd consecutive run) — **NO rating change asserted per the Re-Verify Gate**, snippet reliability remains downgraded. **Facebook** 5/100% INLINE 32nd consecutive run. **The Knot** Haylee L. 1★ body still indexed in Rancho-attributed snippet via name-free query (name-surfacing not asserted, no owner-response indexed, 108 days unreplied); 8/4.5★ carried; direct fetch not attempted (BLOCKER `theknot-direct-fetch`, 17th no-attempt run). **Hipcamp** direct fetch not attempted (BLOCKER `hipcamp-direct-fetch`, 8th no-attempt-cycle run); listing still indexed via WebSearch; voice/data violations carry STALE:2026-05-26. **Airbnb** 403 BLOCKER 50th no-attempt; **Hotels.com** timeout BLOCKER (counter holds 42, cruise-control). Dashboard status: `pending`. No new review-report produced (no new reviews to draft); both carry-forward drafts remain at `brand/review-reports/2026-05-19-review-report.md`. State updated: `brand/review-aggregate.json` + `site/admin/dashboard-state.json` + `run-logs/2026-06-14-review-monitor.md`. Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged.

## 2026-06-16 — Review Monitor RUN_053 (20th quiet sweep; Google snippet 4-run hold at 126; TripAdvisor price held $66–$179; Expedia 8.0 anchor reconfirmed)

- **rancho-review-monitor RUN_053:** Twentieth consecutive quiet platform sweep — no new reviews on any monitored platform, no rating/count drops, no new ≤3★. Cruise-control maintenance run under GOALS.md week-of-5/18 broad Rancho pause (review-monitor not in the Scheduled-Tasks Pause List; monitoring ≠ active work). Two RUN_034 drafts still unposted day 29 in monitor (Cassie Butterfield Google 5★; Haylee L. The Knot 1★ 110 days unreplied); done-log shows no review-related entry since 2026-04-15 22:12. **Re-Verify Gate: 8/8 still_true, 0 resolved** (no done-log RESOLVED line). **TripAdvisor** DIRECT WebFetch — 0/unclaimed unchanged; price range **held $66–$179** (no drift; RUN_052's +$1 floor tick holding). **Google** WebSearch snippet **held at 126** + 4.9★ — 4th consecutive run at 126 (history 126/175/null/126/175/126/126/126/126); non-authoritative; live 130/4.9★ carries from RUN_034 Chrome read (28 days old, STALE). **Expedia** 8.0 anchor **CONFIRMED** on targeted Hotels.com query (23rd consecutive inline run); general query surfaced **9.0 'Wonderful'** (artifact, 4th consecutive run) — **NO rating change asserted per the Re-Verify Gate**, snippet reliability remains downgraded. **Facebook** 5/100% INLINE 34th consecutive run. **The Knot** Haylee L. 1★ body still indexed in Rancho-attributed snippet via name-free query (name-surfacing not asserted, no owner-response indexed, 110 days unreplied); 8/4.5★ carried; direct fetch not attempted (BLOCKER `theknot-direct-fetch`, 19th no-attempt run). **Hipcamp** direct fetch not attempted (BLOCKER `hipcamp-direct-fetch`, 10th no-attempt-cycle run); listing still indexed via WebSearch; voice/data violations carry STALE:2026-05-26. **Airbnb** 403 BLOCKER 52nd no-attempt; **Hotels.com** timeout BLOCKER (counter holds 42, cruise-control; 8.0 cross-confirmed via Expedia targeted query). Dashboard status: `pending`. No new review-report produced (no new reviews to draft); both carry-forward drafts remain at `brand/review-reports/2026-05-19-review-report.md`. State updated: `brand/review-aggregate.json` + `site/admin/dashboard-state.json` + `run-logs/2026-06-16-review-monitor.md`. Committed by explicit path; pre-existing prior-session changes + concurrent-writer artifacts intentionally NOT staged.

## 2026-07-15 — rancho-competitive-weekly
- First competitive run since 6/29 (7/6 didn't fire). Rancho paused per GOALS.md 5/18; task not on pause list → ran.
- **Two genuine ResortPass state changes.** Rancho's own ResortPass listing went LIVE with products (Half-Day $15, Day Pass $20, Pilates and Horses $58, Full Moon Sound Bath $30; 4.9★/45 reviews) after 7 weeks dormant — resolves the 42-day activate-or-kill decision drift. Lucky Arrow's ResortPass also activated (6 products, pool 12–8pm) — reverses "frozen both sides."
- Re-Verify Gate (11 claims): 6 still_true, 1 rotation-CHANGED (Hipcamp set rotated, Lucky Arrow #9→#8, Rancho absent 7th read), 1 Spoon Mountain price-copy wrinkle, 1 UNVERIFIED-on-page (Hotels.com 8th wk), 2 auto-resolved → done-log.
- Rancho SEO side unchanged: 0/4 non-brand; `/corporate-retreats/` ~75d, `/safari-tents-near-austin/` ~80d uncrawled; brand #1 (+ new Tribeza editorial feature on brand SERP).
- Intel card dual-written byte-identical to both improvement-plan.html files (len 10647==10647). competitive-intelligence.md rewritten. 3 done-log RESOLVED lines. Run-log: run-logs/2026-07-15-competitive.md.
